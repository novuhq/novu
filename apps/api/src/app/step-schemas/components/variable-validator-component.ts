...
export class VariableValidatorComponent {
  public searchAndValidatePlaceholderExistence(
    ...
    if (validationStrategy.includes(ValidationStrategyEnum.VALIDATE_MISSING_PAYLOAD_VALUES_FOR_HYDRATION)) {
      const placeholders = this.collectPlaceholders(controlsValues);
      issues = { ...this.validatePlaceholders(placeholders, payload), ...issues };
    }
    if (validationStrategy.includes(ValidationStrategyEnum.VALIDATE_MISSING_CONTROL_VALUES)) {
  ...
  private validateMissingControlValues(controlSchema, controlsValues?: Record<string, unknown>) {
    let issues: Record<string, ControlPreviewIssue[]> = {};
