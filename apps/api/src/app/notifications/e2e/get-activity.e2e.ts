...
import { MessageRepository, NotificationRepository, NotificationTemplateEntity } from '@novu/dal';
...
describe('Get activity - /notifications/:notificationId (GET) #novu-v2', async () => {
  ...
  it('should return traces in activity feed when traces feature flag is enabled', async () => {
    ...
    const { status } = await updateNotification({
      id: message._id,
      status: 'read',
    });
  ...
  it('should return mongo jobs enriched with traces', async () => {
    const triggerResponse = await novuClient.trigger({
      workflowId: template.triggers[0].identifier,
      to: session.subscriberId,
      payload: { name: 'Test User' },
  ...
  it('should fallback to old method when traces are not enriched for unread message', async () => {
    const triggerResponse = await novuClient.trigger({
      workflowId: template.triggers[0].identifier,
      to: session.subscriberId,
      payload: { name: 'Test User' },
