...
describe('Filter topics - /topics (GET)', async () => {
  ...
  it('should retrieve the topic filtered by multiple query param', async () => {
    const topicKey = 'topic-key-2';
    const url = `${BASE_PATH}?subscriberId=${firstSubscriber.subscriberId}&key=${topicKey}&pageSize=2`;
    const response = await session.testAgent.get(url);
