...
describe('EmailOutputRendererUsecase', () => {
  ...
  describe('removeTrailingEmptyLines functionality', () => {
    beforeEach(() => {
      featureFlagsServiceMock.getFlag.resolves(false); // Disable layouts for these tests
      getOrganizationSettingsMock.execute.resolves({ 
        removeNovuBranding: true,
        defaultLocale: 'en-US',
        translationsEnabled: false
    ...
    it('should remove trailing empty paragraphs', async () => {
      const mockTipTapNode: MailyJSONContent = {
        type: 'doc',
        content: [
          {
      ...
      // Should not contain multiple empty paragraph tags at the end
      const emptyParagraphMatches = result.body.match(/<p><\/p>/g);
      expect(emptyParagraphMatches).to.be.null;
    });

    it('should remove trailing empty headings', async () => {
      const mockTipTapNode: MailyJSONContent = {
        type: 'doc',
        content: [
          {
      ...
      const result = await emailOutputRendererUsecase.execute(renderCommand);
      expect(result.body).to.include('Content here');
      
      // Should not contain empty heading tags at the end
      expect(result.body).to.not.include('<h1></h1>');
      expect(result.body).to.not.include('<h2></h2>');
    });

    it('should remove trailing spacer elements', async () => {
      const mockTipTapNode: MailyJSONContent = {
        type: 'doc',
        content: [
          {
      ...
      // Should not contain trailing spacer elements
      const spacerMatches = result.body.match(/height.*md|height.*lg/g);
      expect(spacerMatches).to.be.null;
    });

    it('should handle nested empty elements recursively', async () => {
      const mockTipTapNode: MailyJSONContent = {
        type: 'doc',
        content: [
          {
      ...
      const result = await emailOutputRendererUsecase.execute(renderCommand);
      expect(result.body).to.include('Good content');
      
      // Should not contain empty blockquote elements
      expect(result.body).to.not.include('<blockquote></blockquote>');
      expect(result.body).to.not.include('<blockquote><p></p></blockquote>');
    });

    it('should preserve non-empty elements at the end', async () => {
      const mockTipTapNode: MailyJSONContent = {
        type: 'doc',
        content: [
          {
          ...
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'Last paragraph with content',
              },
            ],
          },
        ],
      ...
      const result = await emailOutputRendererUsecase.execute(renderCommand);
      expect(result.body).to.include('First paragraph');
      expect(result.body).to.include('Last paragraph with content');
    });

    it('should handle mixed empty trailing elements', async () => {
      const mockTipTapNode: MailyJSONContent = {
        type: 'doc',
        content: [
          {
      ...
      const result = await emailOutputRendererUsecase.execute(renderCommand);
      expect(result.body).to.include('Content with substance');
      
      // Should not contain trailing empty elements
      expect(result.body).to.not.include('<h3>   </h3>');
      expect(result.body).to.not.include('<br>');
    });

    it('should handle elements with text-bearing attributes correctly', async () => {
      const mockTipTapNode: MailyJSONContent = {
        type: 'doc',
        content: [
          {
      ...
      const result = await emailOutputRendererUsecase.execute(renderCommand);
      expect(result.body).to.include('Main content');
      expect(result.body).to.include('Click me');
      
      // Should remove empty button but keep the one with content
      const buttonMatches = result.body.match(/Click me/g);
      expect(buttonMatches).to.have.length(1);
    });
  });
});
