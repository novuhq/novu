...
describe('Agent Chat - /agent-chat/conversations #novu-v2', () => {
  ...
  async function setAgentChatHmac(enabled: boolean) {
  ...
  function listConversations(token = subscriberToken, query: { after?: string; before?: string; limit?: number } = {}) {
    return ctx.session.testAgent
      .get('/v1/agent-chat/conversations')
      .query(query)
      .set('Authorization', `Bearer ${token}`);
  }
  ...
  it('should create a conversation on first message when agent is published to agent chat', async () => {
    await linkAgentChat();

    const res = await createConversation({
      agentId: ctx.agentIdentifier,
      text: 'Hello from agent chat',
    });
    ...
    expect(activities.content).to.equal('Hello from agent chat');
    expect(activities.identifier).to.equal(res.body.data.messageId);
    expect(conversation.channels.some((channel) => channel.platform === 'agent_chat')).to.equal(true);
    expect(conversation.channels[0]?.platformThreadId).to.equal(`agent_chat:${res.body.data.identifier}`);
  });
  ...
  it('should allow resume without agentHash when agent-chat HMAC is off', async () => {
    await linkAgentChat();
    await setAgentChatHmac(false);
  ...
  it('should reject missing agentHash with 400 when agent-chat HMAC is on', async () => {
    await linkAgentChat();
    await setAgentChatHmac(true);
  ...
  it('should reject invalid agentHash with 400 when agent-chat HMAC is on', async () => {
    await linkAgentChat();
    await setAgentChatHmac(true);
  ...
  it('should reject resume with missing agentHash when agent-chat HMAC is on', async () => {
    await linkAgentChat();
    await setAgentChatHmac(false);
  ...
  it('should return 401 when Authorization is missing on POST', async () => {
    ...
    const res = await ctx.session.testAgent.post('/v1/agent-chat/conversations').send({
      agentId: ctx.agentIdentifier,
      text: 'No auth',
    });
  ...
  it('should reject GET events for non-agent-chat conversations', async () => {
  ...
  it('should list only the subscriber agent-chat conversations', async () => {
    await linkAgentChat();
  ...
  it('should emit exactly one live AGENT_EVENT per post/edit/delete/typing via adapter callbacks', async () => {
    ...
    // Inbound ack starts typing on agent_chat (PLATFORMS_WITH_TYPING_INDICATOR). Wait for
    ...
    if (!messageActivity.platformMessageId) {
      throw new Error('Expected agent-chat delivery to persist a platform message id');
    }
  ...
  it('should return concurrent durable events in sequence order on the newest page', async () => {
    ...
    expect(createRes.status).to.equal(201);
    const conversation = await waitForConversation(createRes.body.data.identifier);
    const channel = conversation.channels.find((candidate) => candidate.platform === 'agent_chat');
    if (!channel) {
      throw new Error('Expected a agent-chat channel');
    }
