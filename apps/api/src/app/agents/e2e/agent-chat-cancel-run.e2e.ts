...
describe('Agent Chat - cancel run #novu-v2', () => {
  ...
  beforeEach(async () => {
    ...
    const agentRunRegistry = testServer.getService(AgentRunRegistryService);
    sinon.stub(bridgeExecutor, 'execute').callsFake(async (params) => {
      agentRunRegistry.register(String(params.conversation._id), 'bridge');
    });
  ...
  it('should cancel an in-flight dispatch before run-start is ingested', async () => {
    ...
    const cancelRes = await cancelRun(publicId, {
      agentId: ctx.agentIdentifier,
      idempotencyKey: 'cancel-e2e-pre-start',
    });
