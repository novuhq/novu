...
function buildWhatsAppToolApprovalCard(
  toolLabel: string,
  buttons: Array<{ id: string; label: string; style?: 'default' | 'primary' }>
): Record<string, unknown> {
  return {
    type: 'card',
    ...
    children: [
      {
        ...
        children: buttons
          ...
          .map((button) =>
            buildApprovalButton({
              id: button.id,
              label: truncateWhatsAppButtonLabel(button.label),
              value: toolLabel,
              style: button.style,
            })
          ),
...
function buildMcpToolApprovalWhatsAppCard(tool: PendingToolApproval): Record<string, unknown> {
  const toolLabel = mcpToolLabel(tool);

  return buildWhatsAppToolApprovalCard(toolLabel, [
    {
      id: buildToolApprovalActionId(MCP_TOOL_APPROVAL_ACTION_PREFIX, 'deny', tool.toolUseId),
      label: 'Deny',
    },
...
function buildDirectToolApprovalWhatsAppCard(tool: PendingToolApproval): Record<string, unknown> {
  const toolLabel = directToolLabel(tool);
...
function resolveToolApprovalCard(tool: PendingToolApproval, platform?: string): ReplyContentDto {
  const isMcpTool = tool.mcpServerName !== undefined;

  if (platform === AgentPlatformEnum.WHATSAPP) {
    return {
      card: isMcpTool ? buildMcpToolApprovalWhatsAppCard(tool) : buildDirectToolApprovalWhatsAppCard(tool),
    };
  }

  return {
    card: isMcpTool ? buildMcpToolApprovalCard(tool) : buildDirectToolApprovalCard(tool),
  };
}

export function getToolApprovalCard(params: { platform?: string; tool: PendingToolApproval }): ManagedCardDelivery {
  const content = resolveToolApprovalCard(params.tool, params.platform);
  const isMcpTool = params.tool.mcpServerName !== undefined;

  if (params.platform === AgentPlatformEnum.SLACK) {
    return {
      content,
      slackNative: isMcpTool
        ? buildMcpToolApprovalSlackBlocks(params.tool)
        : buildDirectToolApprovalSlackBlocks(params.tool),
    };
  }
