 ...
 */
export function buildPortableWhatsAppApprovalCard(spec: ApprovalCardSpec): Record<string, unknown> {
  const buttons = [
    portableButton({ ...spec.deny, label: truncateWhatsAppButtonLabel(spec.deny.label) }, 'default'),
    portableButton({ ...spec.approve, label: truncateWhatsAppButtonLabel(spec.approve.label) }, 'primary'),
  ];

  if (spec.trust) {
    buttons.push(
      portableButton({ ...spec.trust.tool, label: truncateWhatsAppButtonLabel('Always allow tool') }, 'default')
    );
  }
  ...
  return {
    ...
    children: [
      {
        type: 'actions',
        children: buttons.slice(0, WHATSAPP_MAX_REPLY_BUTTONS),
      },
    ],
  };
}
