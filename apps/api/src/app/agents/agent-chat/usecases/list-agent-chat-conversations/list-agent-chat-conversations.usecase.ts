...
import { ListAgentChatConversationsCommand } from './list-agent-chat-conversations.command';

@Injectable()
export class ListAgentChatConversations {
  constructor(
    private readonly conversationRepository: ConversationRepository,
    private readonly agentRepository: AgentRepository
  ) {}

  async execute(command: ListAgentChatConversationsCommand): Promise<ListAgentChatConversationsResponseDto> {
    ...
    const page = await this.conversationRepository.listConversations({
      ...
      provider: [AgentPlatformEnum.AGENT_CHAT],
    ...
    return {
      data: page.data
        ...
        .filter((item): item is AgentChatConversationMetadataDto => item !== null),
      next: page.next,
  ...
  ): AgentChatConversationMetadataDto | null {
    if (!agentIdentifier) {
      return null;
    }
