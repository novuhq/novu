...
import { AgentMcpServerEntity, AgentMcpServerRepository, McpConnectionRepository } from '@novu/dal';
import { MCP_SERVERS, McpConnectionScopeEnum, McpConnectionStatusEnum } from '@novu/shared';

@Injectable()
export class AgentMcpSessionService {
  constructor(
   ...
   *
   * Anonymous platform turns (no subscriber) receive `[]` because no write path
   * creates `scope: 'agent'` rows today. When OAuth MCPs are enabled but no
   * vault exists yet, one is created so credentials can be bound via `vault_ids`
   * after the subscriber completes in-thread OAuth (dispatch is gated until then).
   */
   ...
   * this supplies connected OAuth MCPs per send. Non-OAuth MCPs on the agent
   * definition are always available and are included here when OAuth MCPs exist
   * so the per-send list stays complete for the turn.
   *
   * Returns `undefined` when no per-send override is needed (no enabled OAuth MCPs).
   */
  async resolveConnectedMcps(params: {
    agentId: string;
    environmentId: string;
    organizationId: string;
    subscriberMongoId?: string;
  ...
  private logOrphanVault(args: {
    connection?: { _id?: string; _organizationId?: string; _environmentId?: string; _subscriberId?: string };
    ourVaultId: string;
