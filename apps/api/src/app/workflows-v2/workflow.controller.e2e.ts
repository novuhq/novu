...
function buildInAppStep(
	overrides: Partial<InAppStepUpsertDto> = {},
): InAppStepUpsertDto {
	return {
...
function buildDigestStep(
	overrides: Partial<DigestStepUpsertDto> = {},
): DigestStepUpsertDto {
	return {
...
function buildEmailStep(
	overrides: Partial<EmailStepUpsertDto> = {},
): EmailStepUpsertDto {
	return {
...
async function createWorkflowAndExpectError(
	...
	const res = await expectSdkExceptionGeneric(() =>
		apiClient.workflows.create(createWorkflowDto),
	);
...
async function createWorkflowAndExpectValidationError(
	...
	const res = await expectSdkValidationExceptionGeneric(() =>
		apiClient.workflows.create(createWorkflowDto),
	);
	expect(res.error).to.be.ok;
	if (expectedPartialErrorMsg) {
		expect(JSON.stringify(res.error?.errors)).to.include(
			expectedPartialErrorMsg,
		);
	}
...
describe("Workflow Controller E2E API Testing #novu-v2", () => {
	...
	describe("Create workflow", () => {
		...
		it("should not allow to create more than 20 workflows for a free organization", async () => {
			...
			const error = await createWorkflowAndExpectError(
				apiClient,
				createWorkflowDto,
			);
		...
		it("should reject workflow creation with invalid JSON schema", async () => {
			...
			const error = await createWorkflowAndExpectValidationError(
				apiClient,
				createWorkflowDto,
			);
			expect(error?.statusCode).to.equal(422);
			expect(JSON.stringify(error)).to.include(
				"payloadSchema must be a valid JSON schema",
			);
		});
	});
	...
	describe("Update workflow", () => {
		it("should update control values", async () => {
			...
			const workflowCreated: WorkflowResponseDto =
			...
			expect(
				(updatedWorkflow.steps[0] as InAppStepResponseDto).controls.values
					.subject,
			).to.be.equal(inAppControlValue);
			expect(
				(updatedWorkflow.steps[1] as EmailStepResponseDto).controls.values
					.subject,
			).to.be.equal(emailControlValue);
		});
		...
		it("should keep the step id on updated ", async () => {
			...
			expect(stepIds1.length).to.equal(
				uniqueStepIds1.length,
				"All step ids should be unique on creation",
			);

			// Add a step of an existing channel at the beginning of the steps array
			workflowCreated.steps = [
				buildInAppStep(),
				...workflowCreated.steps,
			] as any;
			const updatedWorkflow = await updateWorkflow(
			...
			expect(stepIds2.length).to.equal(
				uniqueStepIds2.length,
				"All step ids should be unique after update",
			);
		});
		...
		it("should update user preferences", async () => {
			...
			expect(
				updatedWorkflow.preferences.user,
				JSON.stringify(updatedWorkflow, null, 2),
			).to.be.ok;
			expect(
				updatedWorkflow.preferences?.user?.all.enabled,
				JSON.stringify(updatedWorkflow, null, 2),
			).to.be.false;
	...
	describe("List workflows", () => {
		...
		async function getV2WorkflowIdAndExternalId(prefix: string) {
			...
			const listWorkflowResponse: ListWorkflowResponse = await listWorkflows(
				prefix,
				0,
				5,
			);
			...
			return {
				workflowV2Id,
				workflowId,
				name: listWorkflowResponse.workflows[0].name,
			};
		}

		it("old list endpoint should not retrieve the new workflow", async () => {
			const { workflowV2Id, name } =
				await getV2WorkflowIdAndExternalId("Test Workflow");
			const [, , workflowV0Created] = await Promise.all([
				createV0Workflow(1),
				createV0Workflow(2),
				createV0Workflow(3),
			]);
			let workflowsFromSearch = await searchWorkflowsV0(
				workflowV0Created?.name,
			);
	...
	describe("Promote workflow", () => {
		it("should promote by creating a new workflow in production environment with the same properties", async () => {
			...
			propertiesToCompare.forEach((prop) => {
				expect(prodWorkflow[prop]).to.deep.equal(
					devWorkflow[prop],
			...
			for (const prodStep of prodWorkflow.steps) {
				...
				expect(prodStep.stepId).to.equal(
					devStep.stepId,
					"Step ID should be the same",
				);
				expect(prodStep.controls.values).to.deep.equal(
					devStep.controls.values,
					"Step controlValues should match",
				);
		...
		it("should promote by updating an existing workflow in production environment", async () => {
			...
			expect(prodWorkflowUpdated.workflowId).to.equal(
				prodWorkflowCreated.workflowId,
			);
			...
			["status", "type", "origin"].forEach((prop) => {
				expect(prodWorkflowUpdated[prop]).to.deep.equal(
					prodWorkflowCreated[prop],
					`Property ${prop} should match`,
				);
			});
			...
			expect(prodWorkflowUpdated.steps[0].id).to.equal(
				prodWorkflowCreated.steps[0].id,
			);
			expect(prodWorkflowUpdated.steps[0].stepId).to.equal(
				prodWorkflowCreated.steps[0].stepId,
			);
			...
			expect(prodWorkflowUpdated.steps[1].id).to.not.equal(
				prodWorkflowCreated.steps[1].id,
			);
		...
		it("should throw an error if trying to promote to the same environment", async () => {
			...
			expect(error?.message).to.equal(
				"Cannot sync workflow to the same environment",
			);
		});
		...
		it("should sync payload schema and validation settings to target environment", async () => {
			// Create a workflow with payload schema and validation settings
			const payloadSchema = {
				type: "object",
				properties: {
			...
			const sourceWorkflow = await createWorkflow(apiClient, createWorkflowDto);
			expect(sourceWorkflow.payloadSchema).to.deep.equal(payloadSchema);
			expect(sourceWorkflow.validatePayload).to.be.true;
			...
			// Sync the workflow to production
			const targetWorkflow = await syncWorkflow(
				sourceWorkflow,
				prodEnvironmentId,
			);

			// Verify that payload schema and validation settings were copied
			expect(targetWorkflow.payloadSchema).to.deep.equal(payloadSchema);
			expect(targetWorkflow.validatePayload).to.be.true;
			expect(targetWorkflow.workflowId).to.equal(sourceWorkflow.workflowId);
			expect(targetWorkflow.id).to.not.equal(sourceWorkflow.id);
		});
	});
	...
	describe("Get workflow", () => {
		it("should get by slugify ids", async () => {
			...
			const workflowRetrievedBySlugPrefixAndEncodedInternalId =
				await getWorkflow(slugPrefixAndEncodedInternalId);
			expect(workflowRetrievedBySlugPrefixAndEncodedInternalId.id).to.equal(
				internalId,
			);

			const workflowIdentifier = workflowCreated.workflowId;
			const workflowRetrievedByWorkflowIdentifier =
				await getWorkflow(workflowIdentifier);
		...
		it("should return 404 if workflow does not exist", async () => {
			...
			const novuRestResult = await expectSdkExceptionGeneric(() =>
				apiClient.workflows.get(notExistingId),
			);
	...
	describe("Duplicate workflow", () => {
		it("should duplicate a workflow", async () => {
			...
			expect(duplicatedWorkflow?.description).to.equal(
				workflowCreated.description,
			);
			expect(duplicatedWorkflow?.tags).to.deep.equal(workflowCreated.tags);
			expect(duplicatedWorkflow?.steps.length).to.equal(
				workflowCreated.steps.length,
			);
			...
			expect(duplicatedWorkflow?.preferences).to.deep.equal(
				workflowCreated.preferences,
			);
		});
	...
	describe("Get step data", () => {
		it("should get step by worflow slugify ids", async () => {
			...
			const stepRetrievedBySlugPrefixAndEncodedWorkflowInternalId =
				await getStepData(slugPrefixAndEncodedWorkflowInternalId, stepId);
			expect(stepRetrievedBySlugPrefixAndEncodedWorkflowInternalId.id).to.equal(
				stepId,
			);

			const workflowIdentifier = workflowCreated.workflowId;
			const stepRetrievedByWorkflowIdentifier = await getStepData(
				workflowIdentifier,
				stepId,
			);
		...
		it("should get step by step slugify ids", async () => {
			...
			const slugPrefixAndEncodedStepId = buildSlug(
				`my-step`,
			...
			const stepRetrievedByStepIdentifier = await getStepData(
				internalWorkflowId,
				stepIdentifier,
			);
		...
		describe("Variables", () => {
			it("should get step available variables", async () => {
				const steps = [
					{
						...
						controlValues: {
							...
							editorType: "html",
							subject:
								"Welcome to our newsletter {{subjectText}} {{payload.prefixSubjectText}}",
						},
					},
					{
						...buildInAppStep(),
						controlValues: {
							subject: "Welcome to our newsletter {{inAppSubjectText}}",
						},
					},
				];
				const createWorkflowDto: CreateWorkflowDto = buildWorkflow({
				...
				expect(JSON.stringify(payloadVariables)).to.contain(
					"prefixSubjectText",
				);
			});
			it("should serve previous step variables with payload schema", async () => {
				const steps = [
					buildDigestStep(),
					{
						...buildInAppStep(),
						controlValues: {
							subject: "Welcome to our newsletter {{payload.inAppSubjectText}}",
						},
					},
				];
				const createWorkflowDto: CreateWorkflowDto = buildWorkflow({
				...
				const res = await createWorkflow(apiClient, createWorkflowDto);
				const novuRestResult = await apiClient.workflows.steps.retrieve(
					res.id,
					res.steps[1].id,
				);
				const { variables } = novuRestResult.result;
				const variableList = getJsonSchemaPrimitiveProperties(
					variables as JSONSchemaDto,
				);
				const hasStepVariables = variableList.some((variable) =>
					variable.startsWith("steps."),
				);
	...
	describe("Delete workflow", () => {
		it("should delete a workflow", async () => {
			...
			const { error, successfulBody } = await expectSdkExceptionGeneric(() =>
				apiClient.workflows.delete(workflowId),
			);
	...
	describe("Error handling", () => {
		...
		describe("steps validation", () => {
			it("should throw 400 when name is empty", async () => {
				...
				const overrideDto = {
					steps: [
						{ ...buildEmailStep(), name: "" } as unknown as StepUpsertDto,
					],
				};
				const createWorkflowDto: CreateWorkflowDto = buildWorkflow();
			...
			describe("step control issues", () => {
				it("should return issues for all steps immediately", async () => {
						...
						steps: [
							{
								...
								controlValues: {
									...
									redirect: {
										url: "not-good-url-please-replace",
										target: "_blank",
									},
									primaryAction: {
										label: "primary",
					...
					expect(
						stepData.issues!.controls!["redirect.url"][0].issueType,
					).to.equal("MISSING_VALUE");
					expect(
						stepData.issues!.controls!["primaryAction.redirect.url"][0]
							.issueType,
					).to.equal("MISSING_VALUE");
					expect(
						stepData.issues!.controls!["secondaryAction.redirect.url"][0]
							.issueType,
					).to.equal("MISSING_VALUE");
				});
				...
				it("should always show digest control value issues when illegal value provided", async () => {
					const steps = [
						{
							...buildDigestStep({
								controlValues: { amount: 555, unit: "days" },
							}),
						},
					];
					const workflowCreated = await createWorkflow(
						apiClient,
						buildWorkflow({ steps } as CreateWorkflowDto),
					);
					const step = workflowCreated.steps[0];

					expect(step.issues?.controls?.amount[0].issueType).to.deep.equal(
						StepContentIssueEnum.TierLimitExceeded,
					);
					expect(step.issues?.controls?.unit[0].issueType).to.deep.equal(
						StepContentIssueEnum.TierLimitExceeded,
					);
				});
				...
				it("should always show issues for illegal variables in control values", async () => {
					...
					expect(
						stepData.issues?.controls?.body,
						"Step data should have body issues",
					).to.exist;
					expect(stepData.issues?.controls?.body?.[0]?.variableName).to.equal(
						"{{}}",
					);
					expect(stepData.issues?.controls?.body?.[0]?.issueType).to.equal(
	...
	async function syncWorkflow(
		devWorkflow: WorkflowResponseDto,
	...
	async function updateWorkflowAndValidate(
		...
		for (let i = 0; i < updateRequest.steps.length; i++) {
			...
			if (stepInRequest.controlValues) {
				expect(stepInRequest.controlValues).to.deep.equal(
					updatedWorkflow.steps[i].controls.values,
				);
			}
			...
			if ("id" in stepInRequest) {
				...
				).to.equal(updatedWorkflow.steps[i].slug);
			}
		}

		expect(new Date(updatedWorkflow.updatedAt)).to.be.greaterThan(
			new Date(expectedPastUpdatedAt),
		);
	}

	async function create10Workflows(prefix = "Test Workflow") {
		// eslint-disable-next-line no-plusplus
		for (let i = 0; i < 10; i++) {
	...
	async function createWorkflowAndValidate(
		name = "Test Workflow",
	): Promise<WorkflowResponseDto> {
		const createWorkflowDto: CreateWorkflowDto = buildWorkflow({ name });
		const res = await createWorkflow(apiClient, createWorkflowDto);
	...
	function mapResponseToUpdateDto(
	...
	function assertWorkflowResponseBodyData(
		...
		expect(workflowResponseDto.updatedAt, stringify(workflowResponseDto)).to.be
			.ok;
		expect(workflowResponseDto.createdAt, stringify(workflowResponseDto)).to.be
			.ok;
		expect(workflowResponseDto.preferences, stringify(workflowResponseDto)).to
			.be.ok;
		expect(workflowResponseDto.status, stringify(workflowResponseDto)).to.be.ok;
		expect(workflowResponseDto.origin, stringify(workflowResponseDto)).to.be.eq(
			ResourceOriginEnum.NovuCloud,
		);
		expect(
			Object.keys(workflowResponseDto.issues || {}).length,
			stringify(workflowResponseDto),
		).to.be.equal(0);
	}

	function assertStepResponse(
		workflowResponseDto: WorkflowResponseDto,
		createWorkflowDto: CreateWorkflowDto,
	) {
		// eslint-disable-next-line no-plusplus
	...
	function getJsonSchemaPrimitiveProperties(
			...
			for (const key in schema.properties) {
				...
				if (isPrimitiveType(propertySchema)) {
					...
					properties = properties.concat(
						getJsonSchemaPrimitiveProperties(propertySchema, propertyPath),
					);
				}
			}
		}
		...
		if (schema.items && isJSONSchemaDto(schema.items)) {
			...
			if (isPrimitiveType(schema.items)) {
				...
				properties = properties.concat(
					getJsonSchemaPrimitiveProperties(schema.items, prefix),
				);
			}
		}
	...
	function isPrimitiveType(schema: JSONSchemaDto): boolean {
		...
		return primitiveTypes.includes(
