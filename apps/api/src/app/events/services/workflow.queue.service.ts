...
export class WorkflowQueueService {
  ...
  constructor() {
    ...
    this.worker.on('completed', async (job) => {
      await this.jobRepository.updateStatus(job.data._environmentId, job.data._id, JobStatusEnum.COMPLETED);
    });
    this.worker.on('failed', async (job, e) => {
      await this.jobRepository.updateStatus(job.data._environmentId, job.data._id, JobStatusEnum.FAILED);
      await this.jobRepository.setError(job.data._environmentId, job.data._id, e);
    });
