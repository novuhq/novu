...
describe('TelegramStartCodeService', () => {
  function makeService() {
    ...
    const cacheService = {
      ...
      eval: sinon.stub().callsFake(async (_script: string, keys: string[], args: (string | number | Buffer)[]) => {
        ...
        const mode = args[3];
        const agentIdentifier = args[4];
        const linkScope = parsed.linkScope;
        const modeMatches = linkScope?.mode === mode;
        const agentMatches = mode === 'agent' ? linkScope?.agentIdentifier === agentIdentifier : true;
  ...
  const agentLinkScope = { mode: 'agent' as const, agentIdentifier: 'a' };
  ...
  it('issues a 32-char autolink-safe alphanumeric code and stores JSON payload with TTL', async () => {
    ...
    const { code, expiresAt } = await service.issue({
      ...
      linkScope: { mode: 'agent', agentIdentifier: 'agent-1' },
      integrationId: 'int-1',
      subscriberId: 'sub-1',
    });
    ...
    expect(parsed.linkScope).to.deep.equal({ mode: 'agent', agentIdentifier: 'agent-1' });
    expect(setArgs[2]).to.deep.equal({ ttl: TELEGRAM_START_CODE_TTL_SECONDS });
  });

  it('stores pre-resolved contextKeys on the start-code payload', async () => {
    const { service, cacheService } = makeService();
    ...
    await service.issue({
      ...
      context: { key: 'ignored-when-keys-present' },
    });

    const parsed = JSON.parse(cacheService.set.firstCall.args[1] as string);
    expect(parsed.contextKeys).to.deep.equal(['key:value2']);
    expect(parsed.context).to.equal(undefined);
  });

  it('issues integration-scoped codes without an agent identifier', async () => {
    const { service, cacheService } = makeService();
    ...
    const parsed = JSON.parse(cacheService.set.firstCall.args[1] as string);
    expect(parsed.linkScope).to.deep.equal({ mode: 'integration' });

    const consumed = await service.consumeIfMatches(code, {
      environmentId: 'env-1',
      organizationId: 'org-1',
      integrationId: 'int-1',
      linkScope: { mode: 'integration' },
    ...
    expect(consumed.status).to.equal('consumed');
  });
  ...
  it('consumeIfMatches returns mismatch when link mode differs', async () => {
    const { service, cacheStore } = makeService();
    const { code } = await service.issue({
      environmentId: 'e',
      organizationId: 'o',
    ...
    const result = await service.consumeIfMatches(code, {
      ...scope,
      linkScope: { mode: 'integration' },
    });
