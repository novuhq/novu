...
const TEST_FILE_PATTERN = /\.e2e\.ts$/;
...
function collectWeightedFiles() {
  const candidates = collectTestFiles(path.join(ROOT, 'src'));
...
function printShardSummary(shardIndex, totalShards, shard) {
  console.log(`Running Novu V2 CE E2E shard ${shardIndex}/${totalShards} with ${shard.files.length} files (weight ${shard.weight}).`);
}
