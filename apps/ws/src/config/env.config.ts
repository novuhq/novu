...
// Local override directory at apps/ws/ — resolves identically from compiled
// (apps/ws/dist/config) and ts-node (apps/ws/src/config) contexts. Used by devs
// who mount secrets there (e.g. 1Password Environments). The committed in-tree
// defaults live next to the source. `resolveDotenvPath` picks an explicit
// `.env.<NODE_ENV>` override before falling back to the generic `.env`
