...
import { VerifyPayloadService } from '../../../services/verify-payload.service';
import { GetLayoutCommand, GetLayoutUseCase } from '../get-layout';
import { LayoutDto } from '../get-layout';
...
export class CompileEmailTemplate {
  ...
  public async execute(command: CompileEmailTemplateCommand) {
    ...
    try {
      ...
      throw new PlatformException(e?.message || `Message content could not be generated`);
    }
  ...
  private async loadTemplateContent(name: string) {
    ...
    if (!process.env.E2E_RUNNER) {
      path = '/src/app/workflow/usecases/send-message/compile-email-template';
    }
