...
export class AddDigestJob {
  ...
  private validateDigest(job: JobEntity): void {
    if (job.type !== StepTypeEnum.DIGEST) {
      throw new PlatformException('Job is not a digest type');
    }

    if (!job.digest?.amount) {
      throw new PlatformException('Invalid digest amount');
    }

    if (!job.digest?.unit) {
      throw new PlatformException('Invalid digest unit');
    }
  }
  ...
  private async shouldDelayDigestOrMerge(job: JobEntity): Promise<AddDigestJobResult> {
    ...
    if (matched === 0 && modified === 1) {
      ...
      if (!digest?.amount || !digest?.unit) {
        throw new PlatformException(`Somehow ${job._id} had wrong digest settings and escaped validation`);
      }
