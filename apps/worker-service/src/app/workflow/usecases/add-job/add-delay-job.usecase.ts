...
export class AddDelayJob {
  ...
  public async execute(command: AddJobCommand): Promise<number | undefined> {
    ...
    if (!data) throw new PlatformException(`Job with id ${command.jobId} not found`);
  ...
  private async calculateDelayAmount(data: JobEntity): Promise<number> {
    if (!data.step.metadata) throw new PlatformException(`Step metadata not found`);

    if (data.step.metadata.type === DelayTypeEnum.SCHEDULED) {
      const delayPath = data.step.metadata.delayPath;
      if (!delayPath) throw new PlatformException(`Delay path not found`);
      ...
      if (delay < 0) {
        throw new PlatformException(`Delay date at path ${delayPath} must be a future date`);
      }
