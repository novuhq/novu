...
const getOutlineStyles = (theme, dark) => ({
...
const getSubtleStyles = (theme, dark) => ({
  border: 'none',
  padding: '0 8px',
...
const getLabelStyles = (disabled, variant, dark) => {
  if (disabled) {
    return {};
  }
...
export default createStyles(
  (theme: MantineTheme, { disabled, inherit, variant, pulse }) => {
    const dark = theme.colorScheme === 'dark';
    const loading = getRef('loading');
    let overrides = {};

    if (variant === 'outline') {
      overrides = getOutlineStyles(theme, dark);
    }

    if (variant === 'subtle') {
      overrides = getSubtleStyles(theme, dark);
    }
    ...
    return {
      label: getLabelStyles(disabled, variant, dark),
      root: {
        backgroundImage: colors.horizontal,
        width: inherit ? '100%' : '',
