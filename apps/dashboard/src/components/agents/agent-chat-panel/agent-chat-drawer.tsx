...
export function AgentChatDrawer({ open, onOpenChange, agent }: AgentChatDrawerProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="bg-bg-white flex h-full w-full flex-col gap-0 overflow-hidden p-0 sm:max-w-[500px] [&_[data-close-button]]:hidden"
        onOpenAutoFocus={(event) => event.preventDefault()}
      >
        <div className="border-stroke-soft bg-bg-weak flex h-[76px] shrink-0 items-start gap-2 border-b px-3 py-4">
        ...
        <div className="flex min-h-0 flex-1 flex-col px-3 pt-4">
          <div className="bg-bg-weak flex h-9 shrink-0 items-center gap-2 rounded-lg py-1.5 pr-1.5 pl-2">
