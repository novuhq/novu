...
import { ChatEmptyState, ChatMessageRow, ChatTypingRow } from '@/components/agents/web-chat-panel/web-chat-parts';
import { WebChatSessionBar } from '@/components/agents/web-chat-panel/web-chat-session-bar';
...
function WebChatPanelInner({ agent, showAddToAppCallouts = false, addToAppHref }: WebChatPanelProps) {
  ...
  const { items, reload } = useWebChatConversationList(agent.identifier, testerSubscriberId);
...
function WebChatSurface({
  ...
  conversations: ReturnType<typeof useWebChatConversationList>['items'];
  showAddToAppCallouts: boolean;
  addToAppHref?: string;
