...
import { BlockConfig, createEditorBlocks } from '../../../maily/maily-config';
...
/**
 * Chat cards support a constrained block set: everything here must be expressible
 * as a ChatCard element (text/markdown, lists, divider, image, button, repeat).
 * Email-only blocks (cards, columns, sections, spacers, html) are disabled.
 */
const CHAT_BLOCK_CONFIG: BlockConfig = {
  highlights: {
    ...
    title: 'Highlights',
    blocks: [
      { type: 'button', enabled: true, order: 0 },
      { type: 'divider', enabled: true, order: 1 },
  ...
  allBlocks: {
    ...
    blocks: [
      ...
      { type: 'heading1', enabled: true, order: 1 },
      { type: 'heading2', enabled: true, order: 2 },
      { type: 'heading3', enabled: true, order: 3 },
      { type: 'bulletList', enabled: true, order: 4 },
      { type: 'orderedList', enabled: true, order: 5 },
      { type: 'blockquote', enabled: true, order: 6 },
      { type: 'button', enabled: true, order: 7 },
      { type: 'divider', enabled: true, order: 8 },
      { type: 'image', enabled: true, order: 9 },
      { type: 'repeat', enabled: true, order: 10 },
      { type: 'hardBreak', enabled: true, order: 11 },
      { type: 'digest', enabled: true, order: 12 },
    ],
...
export const ChatBodyBlocks = () => {
  ...
  const editorKey = useMemo(() => {
    ...
    return `chat-vars-${variableNames.length}-${variableNames.slice(0, 100)}-${translationState}`;
  }, [
