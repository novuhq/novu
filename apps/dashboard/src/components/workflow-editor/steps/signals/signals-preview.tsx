...
type SignalsPreviewBody = {
  body?: string;
};

type SignalsPreviewResult = {
  type: string;
  preview?: SignalsPreviewBody;
};

export const SignalsPreview = ({
  isPreviewPending,
  previewData,
  variant = 'default',
}: {
  ...
  const previewResult = previewData?.result as SignalsPreviewResult | undefined;
  const body =
    previewResult?.type === ChannelTypeEnum.SIGNALS && previewResult.preview?.body ? previewResult.preview.body : '';
        ...
        <div className="flex w-full items-start gap-2">
          ...
          <div className="flex w-full flex-col gap-1">
            <div className="flex items-center gap-1">
              <span className="text-foreground-950 text-xs font-bold">Signals</span>
