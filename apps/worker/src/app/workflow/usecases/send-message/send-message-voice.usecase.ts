...
@Injectable()
export class SendMessageVoice extends SendMessageBase {
  ...
  public async execute(command: SendMessageCommand) {
    const overrideSelectedIntegration = command.overrides?.voice?.integrationIdentifier;
    ...
    if (!step.template) throw new PlatformException(`Unexpected error: VOICE template is missing`);
    ...
    if (!content) {
      throw new PlatformException(`Unexpected error: VOICE content is missing`);
    }
  ...
  private async sendErrors(phone, integration, message: MessageEntity, command: SendMessageCommand) {
    ...
    if (!integration) {
      await this.sendErrorStatus(
        message,
        'warning',
        'voice_missing_integration_error',
        'Subscriber does not have an active voice integration',
        command,
        LogCodeEnum.MISSING_VOICE_INTEGRATION
      );
    ...
    if (!integration?.credentials?.from) {
      await this.sendErrorStatus(
        ...
        command,
        LogCodeEnum.MISSING_VOICE_PROVIDER
      );
      await this.executionLogRoute.execute(
        ExecutionLogRouteCommand.create({
          ...ExecutionLogRouteCommand.getDetailsFromJob(command.job),
  ...
  private async sendMessage(
    ...
    try {
      const voiceFactory = new VoiceFactory();
      const voiceHandler = voiceFactory.getHandler(this.buildFactoryIntegration(integration));
      if (!voiceHandler) {
        throw new PlatformException(`Voice handler for provider ${integration.providerId} is  not found`);
      }

      const result = await voiceHandler.send({
      ...
      await this.sendErrorStatus(
        message,
        'error',
        'unexpected_voice_error',
        e.message || e.name || 'Un-expect VOICE provider error',
        command,
        LogCodeEnum.VOICE_ERROR,
        e
      );
