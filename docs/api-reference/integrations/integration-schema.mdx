...
| `primary` | `boolean` | For email and SMS, the default integration when no conditioned integration is selected. The dashboard and the update API unset `primary` when you save `rules`. Creating the first active email or SMS integration can still set `primary` even if `rules` are present. |
| `conditions` | `StepFilterDto[]` | Legacy StepFilter conditions. Ignored when `rules` is also set. Prefer `rules` and the dashboard **Integration conditions** editor. |
| `rules` | `Record<string, unknown>` | JSON Logic evaluated at send time to select this integration. Create and update accept `context.*`, `subscriber.*`, and the listed `workflow.*` fields (`workflowId`, `name`, `description`, `tags`, `severity`). Payload variables cannot be written; already-saved `payload.*` rules still evaluate at send time. Takes precedence over `conditions`. See [Integration conditions](/platform/integrations/integration-conditions). |
