...
| `conditions` | `StepFilterDto[]` | Legacy filter conditions associated with the integration. If both `rules` and `conditions` are present, send-time selection evaluates `rules` and ignores `conditions`. Updating only `conditions` on an integration that already has `rules` does not change routing. Use `rules` instead. |
| `rules` | `Record<string, unknown>` | JSONLogic rules used to select this integration at send time. Supports tenant, context, and subscriber fields. When both `rules` and `conditions` are set, `rules` take precedence. |
