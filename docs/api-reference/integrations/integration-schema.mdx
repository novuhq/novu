...
| `rules` | `Record<string, unknown>` | JSON Logic evaluated at send time to select this integration. Allowed variables are `context.*`, `subscriber.*`, and the listed `workflow.*` fields (`workflowId`, `name`, `description`, `tags`, `severity`). Payload variables are not supported. Takes precedence over `conditions`. See [Integration conditions](/platform/integrations/integration-conditions). |
