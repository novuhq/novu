...
| `rules` | `Record<string, unknown>` | JSON Logic evaluated at send time to select this integration. Allowed variables are `context.*` and `subscriber.*`. Takes precedence over `conditions`. See [Integration conditions](/platform/integrations/integration-conditions). |
