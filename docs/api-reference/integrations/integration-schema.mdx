...
| `conditions` | `StepFilterDto[]` | Legacy StepFilter conditions. Ignored when `rules` is also set. |
| `rules` | `Record<string, unknown>` | JSONLogic used at send time to select this integration. Takes precedence over `conditions`. |
