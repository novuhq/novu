...
description: "Authenticate REST API requests to Novu with your secret key, manage environment credentials, rotate keys, and follow security best practices."
---

The Novu REST API uses API key authentication. Every server-side request must include your <Tooltip headline="Environment" tip="An isolated scope for workflows, subscribers, integrations, and other Novu data, such as Development or Production." cta="Learn more" href="/platform/developer/environments">environment's</Tooltip> <Tooltip headline="Secret key" tip="The private API key used to authenticate server-side requests. Never expose it in client-side code." cta="Learn more" href="/api-reference/authentication">secret key</Tooltip> in the `Authorization` header.
...
<Warning>
  Your account and its API keys are bound to the region you chose at signup. A US secret key will not authenticate against `https://eu.api.novu.co` (and vice versa) — you cannot switch regions by swapping the base URL. To change region, contact [sales@novu.co](mailto:sales@novu.co); see [Data Residency](/platform/additional-resources/security#data-residency) for details.
</Warning>
