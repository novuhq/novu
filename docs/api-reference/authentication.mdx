...
The Novu REST API uses API key authentication. Every server-side request must include your <GlossaryTerm term="environment">environment's</GlossaryTerm> <GlossaryTerm term="secret-key">secret key</GlossaryTerm> in the `Authorization` header.
