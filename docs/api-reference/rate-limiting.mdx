---
title: 'Rate Limiting'
description: "Learn Novu API rate limits by tier, read the RateLimit response headers, and handle 429 responses with exponential backoff and idempotency keys."
---
...
The response also includes a `Retry-After` header with the number of seconds to wait before retrying. See [Errors](/api-reference/errors) for the full error response format.
...
- [Errors](/api-reference/errors) — HTTP status codes and error response format
- [Idempotency](/api-reference/idempotency) — Safely retry requests without creating duplicates
