...
<Note>
  Hightouch is a reverse ETL platform. Instead of running code on each event like [Segment Destination Functions](/guides/analytics/segment), Hightouch queries a model on a schedule and sends an HTTP request for each row that is added, changed, or removed. You map those row changes to Novu API calls. To run the same subscriber and cohort sync from a Databricks notebook, see [Databricks](/guides/analytics/databricks).
</Note>
