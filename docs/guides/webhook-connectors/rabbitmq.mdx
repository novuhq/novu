...
```json
{"payload":{"object":{"subscriberId":"subscriber-123","channel":"email"}},"eventType":"message.sent"}
```

### Novu example

Consumers usually want the event type and the message fields at the top level so they can route without walking the nested payload. Novu message fields are under `payload.object`:
