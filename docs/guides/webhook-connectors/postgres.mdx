...
| `subscriber_id` | `event.payload.object?.subscriberId ?? null` |
| `channel` | `event.payload.object?.channel ?? null` |
...
```sql
SELECT payload #>> '{object,subscriberId}' AS subscriber_id
FROM novu_events
ORDER BY received_at DESC
LIMIT 20;
```
...
| Subscriber or channel values are empty | Message fields are under `payload.object`, not at the top level of `payload`. |
