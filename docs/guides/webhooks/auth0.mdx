...
sidebarTitle: 'Auth0'
description: "Trigger Novu notification workflows from Auth0 events such as password reset, signup, and login alerts by forwarding webhooks to your Novu workflows."
---
...
export async function handleAuth0Event(event: Auth0LogEvent) {
  // Auth0 Success Signup is type "ss". Type "s" is Success Login.
  if (event.type === 'ss') {
    await novu.trigger({
...
<AccordionGroup>
  ...
  <Accordion title="Which Auth0 log type is a successful signup?">
    Successful signup is type `ss`. Type `s` is a successful login. Match on `event.type` rather than free-text descriptions, which can vary.
  </Accordion>
