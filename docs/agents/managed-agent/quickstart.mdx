...
<Note>
  The CLI uses the same REST APIs under the hood. To build this flow into your own product or CI pipeline, see [Generate an agent configuration](/api-reference/agents/generate-an-agent-configuration) and [Create an agent](/api-reference/agents/create-an-agent).
</Note>
