---
title: "Concepts"
description: "Explore connectors, MCP servers, skills, system prompts, tools, and the conversation lifecycle that shape how a managed agent behaves in Novu."
---
...
| AWS Claude (Claude Platform on AWS) | Available |
| AWS Bedrock AgentCore | Coming soon |
| OpenAI | Coming soon |

### Credentials

Each connector needs platform credentials.

**Claude (Anthropic):**

- **API Key**: from the [Anthropic console](https://console.anthropic.com/settings/keys).

Quickstart: [Connect Claude managed agent](/agents/get-started/claude-managed).

**AWS Claude:**

- **API key**, **region**, and **workspace ID** (`wrkspc_…`) from [Claude Platform on AWS](https://docs.aws.amazon.com/claude-platform/latest/userguide/setup.html).

Quickstart: [Connect AWS Claude managed agent](/agents/get-started/aws-claude).

If you want to try things out first, select **Try a demo agent** in `npx novu connect`. That path uses Novu demo credentials with a limited number of free conversations.
...
Asking a human a structured question (`novu_human` / Novu HITL skill) is separate from that MCP Approve/Deny card. See [Add skills](/agents/managed-agent/add-skills#novu-hitl-skill).
...
You can manage agents programmatically with the [Agents API reference](/api-reference/agents/agent-schema). Managed runtime configuration (system prompt, skills, MCP servers) is configured from the dashboard.

<Columns cols={2}>
  <Card
    title="Create an agent"
    href="/api-reference/agents/create-an-agent"
    description="Create a managed or custom code agent in the current environment"
  />
  <Card
    title="List agents"
  ...
  <Card
    title="Send a reply"
    href="/api-reference/agents/send-an-agent-reply"
    description="Send a message into an agent conversation"
  />
  <Card
    title="Agent integrations"
    href="/api-reference/agents/create-an-agent-integration"
    description="Link chat channels to an agent"
  />
</Columns>
