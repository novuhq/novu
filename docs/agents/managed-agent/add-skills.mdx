---
title: "Add skills"
description: "Upload custom SKILL.md files to a managed agent to give Claude domain-specific instructions, tool guidance, and behavior for your product workflows."
---
...
Novu attaches a **Novu HITL** skill when the agent can already load skills: the **Read** built-in is enabled, or at least one other skill is attached. The skill tells the model when to escalate (risky or irreversible actions, real multi-way choices) and when to just reply in the thread.

If Read is off and the agent has no other skills, the `novu_human` tool is still attached. The skill is skipped. Novu does not turn Read on just for HITL.

`novu_human` is not MCP tool approval. When an MCP or provider tool is already parked on an Approve/Deny card, the model should not call `novu_human` to re-gate that same call.
