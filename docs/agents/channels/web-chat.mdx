...
The agent brain, conversation history, tool approval, and dashboard observability match other ACI channels. The React client is [`useWebChat`](/platform/sdks/react/hooks/use-web-chat) from `@novu/react`. `@novu/react` has no `<WebChat />` export. `npx novu connect` can scaffold a [starter UI](/agents/channels/web-chat/starter-ui).
