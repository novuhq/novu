...
To start another chat without a `conversationId` prop, call `startNewConversation()` from the hook. It clears the current timeline and opens an empty chat in place, so the component does not remount. The next `sendMessage` creates the new server conversation.

```tsx
const { startNewConversation } = useWebChat({
  agentId: 'YOUR_AGENT_IDENTIFIER',
});
...
`startNewConversation()` is a no-op when you pass `conversationId` or a shared `conversation` runtime. In those cases, switch chats by changing the `conversationId` you pass in, or remount the component.
