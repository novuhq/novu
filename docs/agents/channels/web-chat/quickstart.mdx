...
Install `@novu/react`. Wrap the app in `NovuProvider`. Call `useWebChat` and render messages with the headless parts API (text, cards, tools, approvals).

Canonical example: https://docs.novu.co/agents/channels/web-chat/quickstart
Starter UI (full assistant-ui template): https://docs.novu.co/agents/channels/web-chat/starter-ui
...
- Render from `messages` and `parts` — do not invent a `<WebChat />` export from `@novu/react`
