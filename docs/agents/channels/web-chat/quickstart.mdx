...
Build a two-way Web Chat in your React app. Novu supplies conversation state, delivery, and the live socket. You render the UI. `useWebChat` is headless.
...
Install `@novu/react` and `@assistant-ui/react`. Wrap the app in `NovuProvider`. Call `useWebChat` and render it with assistant-ui unless this app already has another chat library.

Canonical example: https://docs.novu.co/agents/channels/web-chat/quickstart
Starter UI: https://docs.novu.co/agents/channels/web-chat/starter-ui
Reference UI: https://github.com/novuhq/novu/tree/next/packages/novu/src/commands/connect/templates/web-chat/ts
Theming: https://www.assistant-ui.com/docs/primitives
...
- Default to assistant-ui. If the app already uses a chat kit, keep that kit and wire `useWebChat` into it
- Remap assistant-ui / shadcn CSS variables to the host theme. Do not paste Novu playground colors into an existing design system
- Disable the composer while `isRunning` or `isLoading` is true
- Use TypeScript, no comments, no empty props

NEVER:

- Invent a `<WebChat />` export from `@novu/react`. It does not exist.
...
See [Chat UI](/agents/channels/web-chat/chat-ui) for parts, retry, resume, and reconnect. See [Starter UI](/agents/channels/web-chat/starter-ui) for the assistant-ui template.
...
<Columns cols={2}>
  ...
  <Card href="/agents/channels/web-chat/starter-ui" title="Starter UI" icon="layout">
    assistant-ui template and Novu part mapping.
  </Card>
