...
`messages` is the ordered timeline. Each message has `role` (`user` or `assistant`) and `parts`. Use these fields with any chat UI. For the `novu connect` assistant-ui starter, see [Starter UI](/agents/channels/web-chat/starter-ui).
