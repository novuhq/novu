...
Convert Novu messages into assistant-ui thread messages, then wrap `Thread` in that runtime.
...
`toThreadMessage` maps each Novu `part` to an assistant-ui content item (`text`, `reasoning`, `tool-call`, or `data`). Start with `part.type === 'text'`. Add the cases below as you need them.

Add a Thread UI from assistant-ui (their [Thread](https://www.assistant-ui.com/docs/primitives/thread), or `npx shadcn@latest add @assistant-ui/thread`) if you do not already have one.
