...
- **Existing project** — installs `@novu/framework` (and runtime packages such as `@novu/framework/ai-sdk` when needed), writes Novu env vars, and wires what it can automatically. It may still leave handler or route wiring for you to finish, depending on your stack.

For the guided onboarding flow, follow the [Quickstart](/agents/custom-code-agent/quickstart). To wire manually:

1. Install `@novu/framework` (and `@novu/framework/ai-sdk` if you use the AI SDK).
