...
description: "Build a Pipeliner customer support agent step by step with event handlers, interactive cards, metadata, an LLM call, and conversation resolution."
---
...
For the full provider flow (Slack tokens, install, permissions), see the [Quickstart](/agents/custom-code-agent/quickstart) and [Create an agent](/agents/custom-code-agent/setup-your-agent/create-an-agent). You can also [create an agent](/api-reference/agents/create-an-agent) and [link a provider](/api-reference/agents/create-an-agent-integration) with the REST API.
