...
- Methods for replying, updating metadata, triggering workflows, resolving the conversation, typing indicators, and plan progress
...
Control the typing status Novu shows while your handler runs. Call `ctx.typing()` to set or replace the status text, `ctx.typing()` with no argument to reset to the default **Thinking…**, or `ctx.typing.stop()` to clear it for the current turn.
...
export const myAgent = agent('my-agent', {
  onMessage: async ({ message, ctx }) => {
    ...
    const answer = await searchDocs(message.text ?? '');
...
Typing updates post immediately, like `ctx.reply()`. Platforms that do not support typing indicators ignore the call. A normal turn that ends with `ctx.reply()` clears the status automatically.

### Plan progress

Show a live plan card while your handler runs multi-step work. Call `ctx.plan(title)` to render the card, then use `plan.step()` for scoped steps or manual updates.
...
export const myAgent = agent('my-agent', {
  onMessage: async ({ message, ctx }) => {
    ...
    const order = await plan.step('Fetch order', () => fetchOrder(message.text));
...
The plan card updates live while the handler runs. The handler return value (or `ctx.reply()`) posts the final reply message. The plan auto-finalizes when the handler completes.
