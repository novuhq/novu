...
description: 'Reference for onMessage, onAction, onReaction, onResolve, and onToolApproval handlers and the context object your custom-code agent receives.'
---
...
| `onToolApproval` | A user clicks Approve or Deny on a card posted via `ctx.toolApproval.request()` | Run or skip a gated tool call and reply with the outcome |
...
`onToolApproval` fires when a user clicks Approve or Deny on an approval card that your agent posted with `ctx.toolApproval.request()`. Register it whenever your agent gates tool calls. See [Tool approval](/agents/custom-code-agent/setup-your-agent/tool-approval) for the full flow.
...
export const myAgent = agent('my-agent', {
  onToolApproval: async (decision, ctx) => {
    if (decision.approved) {
      // run the tool
      return `Ran ${decision.toolCall.name}.`;
    }
    return 'Skipped.';
  },
});
...
<Columns cols={2}>
  ...
  <Card icon="shield-check" href="/agents/custom-code-agent/setup-your-agent/tool-approval" title="Tool approval">
    Gate tool calls with a user Approve/Deny card before they run.
  </Card>
