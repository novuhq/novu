...
The same handler API applies no matter which provider sent the event or which model you use. Your app exposes a bridge endpoint where Novu calls your handlers - see [Connecting your app](/agents/custom-code-agent/connecting-your-app) for project structure, `serve()`, and environment variables.
...
[`ctx.emit`](/agents/custom-code-agent/building-blocks/emit) sends a named payload on the turn. It is not a reply and not a signal. Web Chat renders it as a `data` part.
...
- **Workflow to conversation:** Assign an agent on the workflow so chat and email steps send as that agent. When the subscriber replies, Novu opens an agent conversation and hydrates `ctx.notification` with the original payload. See [Workflow-assigned agents](/agents/workflow-agents).
