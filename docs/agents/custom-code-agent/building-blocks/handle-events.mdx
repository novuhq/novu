...
When a user quote-replies to a prior message, Novu sets `message.replyTo` with that message's platform id. Supported on WhatsApp, Telegram, and Microsoft Teams.

```typescript
const quotedMessageId = message.replyTo?.messageId;
```
