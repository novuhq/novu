...
Handlers are provider-agnostic. The same `onMessage`, `onAction`, `onReaction`, `onResolve`, `onToolApproval`, and `onError` APIs run on Slack, Microsoft Teams, WhatsApp, Telegram, and email. What the user sees (buttons vs action links, typing vs eyes reaction) depends on the channel. See [Channels overview](/agents/channels/overview).

| Event surface | Slack | Teams | WhatsApp | Telegram | Email |
| --- | --- | --- | --- | --- | --- |
...
- The originating workflow notification (`ctx.notification`), when the conversation started from a [workflow-assigned agent](/agents/workflow-agents#reply-hydration) send
...
When a user quote-replies to an earlier message, Novu sets `message.replyTo` on the inbound message with the platform message ID of the quoted message. This is available on **WhatsApp**, **Telegram**, and **Microsoft Teams**; it is `undefined` on channels that do not report a quoted message.
...
export const myAgent = agent('my-agent', {
  onMessage: async (message, ctx) => {
    if (message.replyTo) {
      // message.replyTo.messageId: platform message ID the user quoted
    }
  },
});
```

To send your own reply as a native quote, pass [`quoteReply`](/agents/custom-code-agent/building-blocks/reply#quote-reply) to `ctx.reply()`.
