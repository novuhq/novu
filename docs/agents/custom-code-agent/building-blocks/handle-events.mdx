...
When the user quote-replied to a prior message on WhatsApp, Microsoft Teams, or Telegram, Novu sets `message.replyTo.messageId` to the platform id of the quoted message. Use it with [`quoteReply`](/agents/custom-code-agent/building-blocks/reply#quote-reply) on outbound replies. Slack inbound quote context is not exposed yet.

```typescript
if (message.replyTo?.messageId) {
  // User replied to a specific prior message — not just the thread
}
```
