...
| Quote reply | Plain post | Plain post | Native | Native | — |
...
Pass `quoteReply` to pin the reply to a specific inbound message. WhatsApp and Telegram render a native quote. Other channels send a normal post.

```typescript
await ctx.reply('Your order shipped today.', { quoteReply: message });
```
