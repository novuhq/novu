...
Quote-reply links your outbound message to a specific prior message. WhatsApp and Telegram show the platform native quote UI. Other channels receive a normal post.

### Outbound

Pass `quoteReply` in the second argument to `ctx.reply()`, or return it from the handler:

```typescript
onMessage: async (message, ctx) => {
  await ctx.reply('Your answer', { quoteReply: message });
  // or: { quoteReply: { messageId: message.platformMessageId } }
};

// return value:
return { content: 'Your answer', quoteReply: message };
```

### Inbound

When the user quote-replied to a prior message, read `message.replyTo?.messageId` (WhatsApp, Microsoft Teams, Telegram):

```typescript
if (message.replyTo) {
  await ctx.reply('Checking that message…', {
    quoteReply: { messageId: message.replyTo.messageId },
  });
}
```

See [Handlers and context](/agents/custom-code-agent/building-blocks/handle-events#quote-reply-context) for where inbound fields are documented.

### Agent runtimes

Quote-reply applies to self-hosted agents that run on your bridge endpoint. Managed agents do not support outbound quote-reply today.

| Runtime | Outbound API | Inbound field |
| --- | --- | --- |
| Custom code agent (`@novu/framework`) | `ctx.reply(..., { quoteReply })` | `message.replyTo` |
| Chat SDK bridge (`@novu/chat-sdk-adapter`) | `thread.reply(inboundMessage, text)` | `message.raw.replyTo` |
| Managed agent | Not supported | Not exposed to handlers |

For Chat SDK bridge agents, see the `@novu/chat-sdk-adapter` README on npm or GitHub.
