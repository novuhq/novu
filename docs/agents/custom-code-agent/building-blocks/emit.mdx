...
description: "Send a named payload from a custom code agent handler with ctx.emit. Web Chat renders it as a data part."
sidebarTitle: Emit
---
...
| --- | --- |
| [Web Chat](/agents/channels/web-chat) | A `data` part on the in-flight assistant message |
...
Web Chat adds each event to the assistant message as `part.type === 'data'`. The fields are `name` and `data`.

Render the parts you support. Skip the rest. The part stays on the message.

Client walk: [Custom data parts](/agents/channels/web-chat/chat-ui#custom-data-parts).
...
<Columns cols={2}>
  ...
  <Card icon="list" href="/agents/channels/web-chat/chat-ui#custom-data-parts" title="Web Chat data parts">
