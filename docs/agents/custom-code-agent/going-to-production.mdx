...
description: "Run a custom code agent locally, test it against a development environment, and deploy the bridge to production with monitoring and secure configuration."
---
...
```bash
npx novu@latest dev --port <bridge_application_port> --headless
```

- **bridge_application_port** - Port where your scaffolded project runs. Defaults to 4000.
- **headless** - Runs the CLI without opening the Dashboard in the browser.
