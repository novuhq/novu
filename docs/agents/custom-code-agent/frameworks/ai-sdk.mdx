...
Convert conversation context into AI SDK `ModelMessage[]`. Pass `ctx` so a workflow-origin notification (`ctx.notification`) is prepended as an assistant row with capped notification text and the full JSON payload. See [Workflow-assigned agents](/agents/workflow-agents#reply-hydration) for how that notification is assigned and hydrated.
...
- Wrap `toModelMessages()` in `hydrateUnreachableAttachmentUrls` before `generateText` / `streamText`. Public HTTPS signed URLs stay as URLs; local or HTTP URLs (for example LocalStack) are fetched and inlined as bytes so hosted providers can read the file.
- Pass `ctx.history` instead of `ctx` to skip automatic origin injection when handling typed fields with `isFromWorkflow`:
