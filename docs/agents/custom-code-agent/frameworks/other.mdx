...
description: "Build custom code agents with @novu/framework and any LLM stack — LangChain, OpenAI SDK, or your own integration."
sidebarTitle: Other
---


Import `agent` from `@novu/framework` when you are not using `@novu/framework/ai-sdk`. Call your LLM, orchestration library, or business logic yourself, then reply through Novu's handler API. Use this path for LangChain, the OpenAI SDK, Anthropic SDK, or any stack Novu does not ship a dedicated adapter for yet.
...
For handlers, replies, signals, typing, and tool approval, see [Building blocks](/agents/custom-code-agent/building-blocks/handle-events). The sections below cover what's specific to wiring your own LLM stack — return types, manual tool approval, and differences from the AI SDK path.
...
## Key differences from the AI SDK path

| | AI SDK (`@novu/framework/ai-sdk`) | Other frameworks (`@novu/framework`) |
| --- | --- | --- |
| LLM calls | Return `generateText()` / `streamText()` from `onMessage` | You call the model or library yourself |
| History mapping | `toModelMessages(ctx.history)` | Map `ctx.history` to your library's format |
| Tool approval | `needsApproval: true` on AI SDK tools — Novu auto-resumes the tool loop after the click | `ctx.toolApproval.request()` — you run the tool in `onToolApproval` |
| MCP tools | Wire MCP clients inside `onMessage` — see [AI SDK](/agents/custom-code-agent/frameworks/ai-sdk#mcp-tools) | Same manual wiring through your stack |

There is no automatic tool-loop resume on this path. After the user approves or denies, implement `onToolApproval` to run the tool and post the result.
