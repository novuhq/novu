...
| `{ messages: BaseMessage[] }` | Novu delivers the final assistant text from your pre-invoked agent or graph. Wrap history with `hydrateUnreachableAttachmentUrls` before invoke - see [Invoke your own agent](#invoke-your-own-agent) |
...
Convert conversation context into LangChain `BaseMessage[]` when you invoke an agent or graph yourself. Pass `ctx` so a workflow-origin notification (`ctx.notification`) is prepended as an `AIMessage` with capped notification text and the full JSON payload. See [Workflow-assigned agents](/agents/workflow-agents#reply-hydration) for how that notification is assigned and hydrated.

```typescript
import { hydrateUnreachableAttachmentUrls, toLangChainMessages } from '@novu/framework/langchain';

const messages = await hydrateUnreachableAttachmentUrls(toLangChainMessages(ctx));
```
...
- Returning a `LangChainAgentConfig` already hydrates unreachable attachment URLs (http/localhost) before invoke. If you call `invoke()` yourself, wrap `toLangChainMessages()` in `hydrateUnreachableAttachmentUrls`. Public HTTPS signed URLs stay as URLs; local or HTTP URLs are fetched and inlined as base64 so hosted providers can read the file.
- Pass `ctx.history` instead of `ctx` to skip automatic origin injection when handling typed fields with `isFromWorkflow`; prepend your own `AIMessage` instead.
...
import { agent, hydrateUnreachableAttachmentUrls, toLangChainMessages } from '@novu/framework/langchain';
...
export const supportAgent = agent('support-bot', {
  onMessage: async (message, ctx) => {
    const result = await billingAgent.invoke({
      messages: await hydrateUnreachableAttachmentUrls(toLangChainMessages(ctx)),
    });
...
Set `needsApproval` on `LangChainAgentConfig`. When the model calls a gated tool, Novu posts the Approve / Deny card and pauses the turn. After the user decides, Novu re-runs `onMessage` - `toLangChainMessages(ctx)` replays the approval cycle so the agent continues with no extra code from you.
