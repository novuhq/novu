...
Import `agent` from `@novu/framework/langchain`, then return a `LangChainAgentConfig` from `onMessage`. Prefer a statically imported chat model instance (works with Next.js Turbopack without extra config):
...
Model strings like `model: 'openai:gpt-4o'` also work. On Next.js, list LangChain packages in `serverExternalPackages` — see [Next.js and Turbopack](#nextjs-and-turbopack).
...
1. Pass a statically imported chat model instance (recommended):

```typescript
import { ChatOpenAI } from '@langchain/openai';

return { model: new ChatOpenAI({ model: 'gpt-4o' }), system: '...' };
```

2. Or keep model strings and externalize LangChain in `next.config.mjs`:
...
`npx novu connect --runtime langchain` scaffolds this `serverExternalPackages` list for you.
