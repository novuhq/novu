...
const messages = await hydrateUnreachableAttachmentUrls(toLangChainMessages(ctx.history));
```
...
export const supportAgent = agent('support-bot', {
  onMessage: async (message, ctx) => {
    const result = await billingAgent.invoke({
      messages: await hydrateUnreachableAttachmentUrls(toLangChainMessages(ctx.history)),
    });
