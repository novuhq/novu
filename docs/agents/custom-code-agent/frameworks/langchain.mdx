...
Convert conversation context into LangChain `BaseMessage[]` when you invoke an agent or graph yourself. Pass `ctx` so a workflow-origin notification (`ctx.notification`) is prepended as an `AIMessage` — capped JSON, framed as data not instructions:

```typescript
import { toLangChainMessages } from '@novu/framework/langchain';

const messages = toLangChainMessages(ctx);
```
...
- Passing `ctx.history` instead of `ctx` skips origin injection. Use that when you cherry-pick typed fields with `isFromWorkflow`, then prepend your own `AIMessage`. Do not call `toLangChainMessages(ctx)` after a custom prepend — you would inject the origin twice.
...
export const supportAgent = agent('support-bot', {
  onMessage: async (message, ctx) => {
    const result = await billingAgent.invoke({
      messages: toLangChainMessages(ctx),
    });
