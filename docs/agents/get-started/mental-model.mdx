---
title: "Mental model"
description: "Trace how inbound messages flow from Slack, Teams, WhatsApp, email, or in-app Web Chat through Novu's agent communication infrastructure to your agent code and back."
---
...
<Frame caption="Custom code agent message flow from a connected platform through Novu to your agent brain and back.">
  ![Custom code agent](/images/agents/custom-code-agent/flow.png)
</Frame>

Read more on how to [connect a custom code agent](/agents/get-started/ai-sdk).

  </Tab>
  <Tab title="Managed agent">
...
<Frame caption="Managed agent message flow where the platform manages intelligence and system tools.">
  ![Managed agent](/images/agents/managed-agent/flow.png)
</Frame>

Read more on how to [connect a managed agent](/agents/managed-agent/overview).
...
A conversation can include multiple participants. The agent is one participant in the conversation, but it is not the conversation itself. This distinction matters when you inspect conversations in the dashboard or build agent behavior around conversation state. A Slack thread, email thread, in-app Web Chat thread, or other provider-specific thread maps to a conversation in Novu automatically.
