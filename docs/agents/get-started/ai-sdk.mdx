...
`toModelMessages(ctx)` already includes the current inbound message, and prepends workflow-origin notification data when `ctx.notification` is set. Returning `generateText(...)` delivers the reply. Do not call `ctx.reply()` on this path.
</Step>
...
<AccordionGroup>
  ...
  <Accordion title="How do I pass conversation history to the model?">
    Use `toModelMessages(ctx)`. Do not append the inbound `message` again. The current message is already included. Passing `ctx` also prepends workflow-origin notification data when the conversation was opened from a workflow send.
  </Accordion>
...
- Use `toModelMessages(ctx)` for conversation context
