...
description: "Apply conditional display logic to notifications in the Novu Inbox component to show or hide items based on subscriber, workflow, or payload data."
---
...
You can conditionally display customized notification based on [workflow tags](/platform/workflow/configure-workflow#tags). These tags are defined in your workflow and are accessible via `notification.tags` property.

<Note>
  Inbox filters use the tags copied onto the notification at create time. Changing workflow tags later does not
  update historical notifications. See [Tags](/platform/workflow/configure-workflow#tags).
</Note>
