...
The Inbox animates its state changes. Menus and the Inbox popover grow out of the button that opened them. The notifications and preferences pages slide into each other. A notification you archive or snooze collapses out of the list instead of disappearing.
...
Animated elements carry `data-state="open"` while they enter and `data-state="closed"` while they leave. To remove the animation from a single element, set `animation: 'none'` for its key in `appearance.elements`.
