...
- OAuth access, or a [Novu Cloud API key](https://dashboard.novu.co/settings/api-keys)
...
Connecting with **OAuth** is recommended - you don't copy or paste a key; your client signs you in and picks up your organization automatically. If your client doesn't support OAuth, use a [Novu Cloud API key](#api-key-fallback) instead.
...
| **Authentication** | OAuth (no header), or a [Novu Cloud API key](#api-key-fallback) if your client doesn't support OAuth |
...
Use a Novu Cloud API key for clients without OAuth support, `mcp-remote` / stdio setups, and autonomous agents. Copy your secret key from the [Novu Dashboard](https://dashboard.novu.co/settings/api-keys) and send it as a bearer token:
...
<Warning>
  The hosted Novu MCP endpoints connect to Novu Cloud only. They do not proxy requests to a self-hosted Novu API,
  even when you provide an API key from that instance. Novu does not currently publish a self-hosted account MCP
  server. Self-hosted users can still connect the separate read-only
  [Novu documentation MCP server](https://docs.novu.co/mcp).
</Warning>
...
<Note>
  Every tool accepts an optional `environmentId` parameter - see [Environments](#environments). Agent tools use the agent's identifier (slug), not the Mongo `_id`. Conversation tools on this MCP server require Novu Cloud. Self-hosted Enterprise can still use Conversations in the dashboard and API, but not through `mcp.novu.co`.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Conversations return 404">
    ...
    **identifier** (slug) as `agentId`, not a Mongo `_id`.
  </Accordion>

  <Accordion title="Self-hosted Novu">
    The account-management MCP server is not currently available for self-hosted Novu. `https://mcp.novu.co/` and
    `https://eu.mcp.novu.co/` connect to Novu Cloud and do not accept a self-hosted API URL. Use the
    [Agent Toolkit](/platform/build-with-ai/agent-toolkit) with `backendUrl` set to your API, or call the REST API
    directly. You can still use the [Novu documentation MCP server](https://docs.novu.co/mcp) to search these docs.
  </Accordion>
