...
When using the Next.js Inbox (`@novu/nextjs`), the same React version requirements apply. You also need `react-dom` at a matching version when rendering the Inbox UI.
