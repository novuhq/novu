...
Add real-time in-app notifications to your application with the Novu <Tooltip headline="Inbox" tip="Novu's in-app notification UI and client SDK component that delivers in_app messages, unread counts, and preferences." cta="Set up Inbox" href="/platform/inbox">Inbox</Tooltip> component. Install a client SDK such as [`@novu/react`](/platform/sdks/react), render the [`<Inbox />`](/platform/inbox/setup-inbox) component with your <Tooltip headline="Application identifier" tip="The public, environment-scoped key used by client SDKs and the Inbox to connect to Novu. Safe to expose in frontend code.">application identifier</Tooltip> and <Tooltip headline="Subscriber ID" tip="The unique identifier for a notification recipient in your application, typically the user ID in your database.">subscriber ID</Tooltip>, and Novu handles delivery, unread counts, and <Tooltip headline="Preference" tip="Rules that control whether notifications are sent on a channel, at workflow, subscriber, or subscription scope." cta="Learn more" href="/platform/concepts/preferences">preferences</Tooltip>.
