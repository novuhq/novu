...
Novu Cloud bills by [workflow runs](/platform/account/billing#what-is-a-workflow-run): one execution of a workflow for one subscriber. Usage is counted when Novu creates a **Notification** and **workflow run** for that subscriber, not when you call the trigger API.
...
After triggering a workflow to a topic:

- **Activity / workflow runs**: Subscribers filtered by Context or subscription conditions should **not** appear as new workflow runs for that trigger.
- **Passing subscribers only**: If a topic has 100 subscriptions but only 20 match Context and conditions, expect **20** workflow runs, not 100.

Using subscription Context and conditions to narrow topic fan-out can reduce billable workflow runs when many subscribers share a topic but only some should receive a given trigger.
