...
Subscription conditions use [JSON Logic](https://jsonlogic.com/), the same rule format as [step conditions](/platform/workflow/add-and-configure-steps/step-conditions). Reference condition variables with `{ "var": "<namespace>.<path>" }`.

### Condition variables

These namespaces are available when a condition is evaluated at trigger time:

| Namespace | Source | Examples |
| --- | --- | --- |
| `payload.*` | Trigger [payload](/platform/concepts/trigger) | `payload.tier`, `payload.status`, `payload.category` |
| `subscriber.*` | Recipient profile | `subscriber.email`, `subscriber.locale`, `subscriber.data.plan` |
| `workflow.*` | Workflow metadata | `workflow.name`, `workflow.tags`, `workflow.severity` |
| `context.*` | [Context](/platform/concepts/contexts) on the trigger | `context.tenant.data.plan`, `context.tenant.id` |
| `steps.<stepId>.*` | Prior step results | `steps.in-app-step.read`, `steps.digest-step.eventCount` |

<Note>
  Topic subscription conditions are evaluated at **trigger time** when fanning out a workflow to a topic. At this point, `payload`, `subscriber`, `workflow`, and Context (`context.*`) are in scope. **`steps.*` is not available** at this stage because workflow steps have not executed yet. Use [step conditions](/platform/workflow/add-and-configure-steps/step-conditions) when logic depends on prior step outcomes (for example, send email only if in-app was not read).
</Note>

<Warning>
  Do not confuse **Context** (the Novu feature, referenced as `context.*` in conditions) with the general idea of "execution context." Context is explicit metadata you pass on triggers and subscriptions. [Payload](/platform/concepts/contexts#context-vs-payload) holds event-specific data for a single run.
</Warning>

### Syntax rules

- Use dot paths in `{ "var": "..." }`. For example, `{ "var": "payload.tier" }` or `{ "var": "subscriber.data.plan" }`.
...
- Do **not** omit the namespace prefix (for example `{ "var": "tier" }`). Always use the full path such as `payload.tier`.

### Examples by variable source

**Payload (tier is premium):**
...
**Context (tenant plan from Context data):**

```json
{
  "===": [{ "var": "context.tenant.data.plan" }, "enterprise"]
}
```

**Payload + subscriber combined:**
...
| Field | Behavior |
| --- | --- |
| `condition` | JSON Logic rule evaluated against the available variables. When present, **`enabled` is ignored** during evaluation. |
...
```
  ...
  <Step title="Novu matches Context, then evaluates conditions">
    Novu first selects topic subscriptions whose stored Context exactly matches the trigger Context (including the default/no-Context case). For each matching subscription, Novu evaluates the JSON Logic condition against `payload`, `subscriber`, `workflow`, and Context variables. Subscriptions that pass receive the notification; others are skipped.
  </Step>
</Steps>
