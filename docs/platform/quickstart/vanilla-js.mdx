...
description: "Integrate the Novu Inbox component into a Vanilla JS or plain HTML project and start showing real-time in-app notifications without a frontend framework."
...
<html lang="en">
  ...
  In above code, replace `NOVU_APPLICATION_IDENTIFIER` with actual <Tooltip headline="Application identifier" tip="The public, environment-scoped key used by client SDKs and the Inbox to connect to Novu. Safe to expose in frontend code.">applicationIdentifier</Tooltip> value and `NOVU_SUBSCRIBER_ID` with actual <Tooltip headline="Subscriber ID" tip="The unique identifier for a notification recipient in your application, typically the user ID in your database.">subscriberId</Tooltip> value.
