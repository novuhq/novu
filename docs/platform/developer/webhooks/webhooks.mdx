---
title: 'Webhooks'
description: "Learn how Novu webhooks work, including event types, payload schemas, HMAC signature verification, retries, and how to configure endpoints in the dashboard."
---
...
Novu webhooks allow you to receive notifications when specific events occur in your Novu account. These events include workflow updates, subscriber preference changes, message delivery status updates, and inbound email received on your verified domains.

Every payload is defined in the [OpenAPI specification](https://api.novu.co/openapi.json) under `x-webhooks`, and the **Event Catalog** on the [Webhooks](https://dashboard.novu.co/webhooks) page renders those schemas with example bodies. See [Event types](/platform/developer/webhooks/event-types) for the field-level reference.
...
- **email.received**: Triggered when Novu receives inbound mail matching an Inbound Email webhook route. See [Email events](/platform/developer/webhooks/event-types#email-events) for the payload schema and [Inbound Email](/platform/inbound-email/overview) for setup.
...
See [Workflow events](/platform/developer/webhooks/event-types#workflow-events) for the payload schema.

#### Message Events

- **message.sent**: Triggered when Novu sends the message to the delivery provider.
- **message.delivered**: Triggered when a message delivery provider acknowledged the message delivery to the end receiving client.
- **message.failed**: Triggered when Novu tried to send the message to the delivery provider and it got failed.
- **message.seen**: Triggered when a message is seen.
- **message.read**: Triggered when a message is read.
- **message.unread**: Triggered when a message is unread.
- **message.archived**: Triggered when a message is archived.
- **message.unarchived**: Triggered when a message is unarchived.
- **message.snoozed**: Triggered when a message is snoozed.
- **message.unsnoozed**: Triggered when a message is unsnoozed.
- **message.deleted**: Triggered when a message is deleted.

See [Message events](/platform/developer/webhooks/event-types#message-events) for the payload schema.

#### Preference Events

- **preference.updated**: Triggered when subscriber preference is updated. See [Preference event](/platform/developer/webhooks/event-types#preference-event) for the payload schema.
...
4. For a **Webhook** endpoint, enter the URL. For a connector, follow its destination-specific setup from the [connectors overview](/platform/developer/webhooks/connectors).
