...
3. **Evaluate**: The system checks the conditions and preferences of each matching subscription against the condition variables.
...
To create subscriptions from your backend, from the browser with `@novu/js`, or in React with hooks and components (including JSON Logic conditions), see [Manage topic subscriptions](/platform/subscription/manage-topic-subscriptions).