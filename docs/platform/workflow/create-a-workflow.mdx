...
The dashboard saves workflow edits automatically in the [Development environment](/platform/developer/environments). You cannot disable auto-save, view an earlier workflow version, or restore a previous version.

[Production](/platform/developer/environments#types-of-environments) is view-only for workflows. Make the change in Development, then [publish](/platform/developer/environments#publish-changes-to-other-environments) it to Production. If you need a reversible change, duplicate the workflow first and test the copy before you publish.
