...
description: 'Trigger Novu workflows via the Event API for individual subscribers, topics, or every subscriber, and attach payload and context data at trigger time.'
---
...
To trigger a workflow for every subscriber in the current environment, use the dedicated [Broadcast event to all](/api-reference/events/broadcast-event-to-all) endpoint (`POST /v1/events/trigger/broadcast`). This is useful for environment-wide announcements such as maintenance windows or platform-level notices.
...
await novu.triggerBroadcast({
  workflowId: "system-announcement",
...
<Note>
  Broadcasting to all subscribers is an API-only operation — the Novu dashboard does not expose a "trigger for everyone" button. Test triggers in the dashboard target a single subscriber. For repeated announcements to a defined audience (paying customers, beta testers, and so on), prefer a [topic trigger](#trigger-a-workflow-to-a-topic) so you can manage membership explicitly.
</Note>
