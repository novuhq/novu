...
When you pass a rich context object with a `data` field, Novu upserts the stored context `data` if the context already exists - similar to subscriber fields on the `to` object. Passing only a string identifier references the context without changing stored `data`.
...
## Broadcast a workflow to all subscribers

To send a workflow to every subscriber in the current environment without listing them, call the broadcast endpoint. Broadcast is available only through the API and SDKs. The Novu dashboard does not have a "send to all" button; the dashboard's **Trigger workflow** action always requires an explicit subscriber.

The broadcast request accepts the workflow identifier and an optional `payload`; you do not pass a `to` field.
...
await novu.triggerBroadcast({
  workflowId: "workflow_identifier",
  payload: { message: "Scheduled maintenance tonight at 22:00 UTC." },
});
```
  </Tab>
  <Tab title="cURL">
...
Novu still bills each resolved subscriber as a separate workflow event, and each subscriber's channel preferences still apply. For targeted announcements to a defined audience, prefer [triggering to a topic](#trigger-a-workflow-to-a-topic).

See the [Broadcast event to all](/api-reference/events/broadcast-event-to-all) API reference for the full request schema.
