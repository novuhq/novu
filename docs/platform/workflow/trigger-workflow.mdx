...
From here, you can extend event triggering by attaching context data, overriding the [workflow-assigned agent](/agents/workflow-agents), or targeting multiple subscribers using topics.

## Override the assigned agent

If the workflow has an agent assigned in **Configure workflow**, that agent is the default for every trigger. Pass `agentId` on the trigger to change that for a single execution:

| `agentId` | Behavior |
| --- | --- |
| Omitted | Use the workflow-assigned agent |
| Public agent identifier | Route this execution through that agent |
| `null` | Disable agent routing for this execution |

`agentId` is the public agent slug, not the Mongo `_id`. See [Workflow-assigned agents](/agents/workflow-agents#override-per-trigger).
...
To skip agent routing for one run while leaving the workflow assignment in place, pass `agentId: null`.
