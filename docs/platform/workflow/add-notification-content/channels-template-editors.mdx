...
description: "Design and configure notification content for email, in-app, push, SMS, and chat channels using the Novu template editors in the dashboard."
---
...
- **SMS**: Body field only
- **Chat**: Block editor for rich cards, or a text editor for plain text / Liquid
...
- HTML entities. The five HTML-reserved characters are escaped to their entity form when rendered:
  ...
  If your payload contains any of these characters in a value that must render literally (for example a filename like `Report & Summary.pdf`), the sanitized output will show the entity form in email and SMS content. To avoid this without disabling sanitization, pass a display-safe value from your application code before triggering the workflow (for example, replace `&` with `and`, or use a separate display field such as `Report and Summary.pdf`).
...
Sanitization is configured per step. Open the step in the workflow editor and toggle **Disable content sanitization** to opt a specific step out of sanitization.
...
The Chat template editor supports a **block editor** (structured cards) and a **text editor** (plain text / Liquid). Existing text steps stay on the text editor, and new empty steps default to the block editor.

For supported blocks, preview, backward compatibility, and how this relates to provider overrides, see [Writing chat templates](/platform/integrations/chat/writing-chat-template).
