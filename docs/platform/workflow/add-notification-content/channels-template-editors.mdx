...
- HTML entities. The five HTML-reserved characters are escaped to their entity form when rendered:
  ...
  If your payload contains any of these characters in a value that must render literally (for example a filename like `Report & Summary.pdf`), the sanitized output will show the entity form in email and SMS content. To avoid this without disabling sanitization, normalize the value in your application code before triggering the workflow (for example, replace `&` with `and`, or URL-encode the value).
