...
<Note>
  Integration conditions are a separate control. They choose which provider integration sends the message after the step runs. See [Integration conditions](/platform/integrations/integration-conditions).
</Note>
...
<Warning>
  Custom properties under `subscriber.data.*` do not have a declared schema. The condition builder therefore treats
  them as strings, even when a subscriber profile currently stores a number or boolean. Numeric operators such as
  `<`, `>`, `between`, and `not between` are not available for these properties.

  When you need typed comparisons, include the value in the workflow payload, declare its number or boolean type in
  the [payload schema](/platform/workflow/configure-workflow#payload-schema), and create the condition against that
  `payload.*` field.
</Warning>
