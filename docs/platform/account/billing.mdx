---
title: 'Billing'
description: "Understand how Novu Cloud billing works, including workflow runs, billing cycles, plan tiers, on-premises Enterprise pricing, and managing your subscription."
---
...
- Triggering a workflow to a **topic** counts as **one workflow run per subscriber whose topic subscription passes** Context matching and JSON Logic conditions during fan-out. Subscribers filtered out before a workflow run is created are not billed. A topic with 100 subscriptions where only 20 pass creates **20** workflow runs. See [Billing and workflow runs](/platform/subscription/manage-topic-subscriptions#billing-and-workflow-runs).
...
Only **organization Owners** can view billing details, change plans, and manage payment methods. Admins, Authors, and Viewers can use the dashboard for workflows, subscribers, and integrations, but they cannot open billing settings or complete checkout. See the [roles and permissions matrix](/platform/account/roles-and-permissions#permissions-matrix) for the full breakdown.

<Note>
  If you need billing access, ask an organization **Owner** to update your role from **Settings → Team** in the dashboard. The Billing tab remains visible for all members, but only Owners can manage subscriptions.
</Note>
...
Members without the Owner role still see the **Billing** tab, but the page explains that billing is restricted and links to [team settings](https://dashboard.novu.co/settings/team) and the [roles and permissions](/platform/account/roles-and-permissions) docs.

From billing settings, Owners can:
...
<AccordionGroup>
  <Accordion title="Which roles can access billing?">
    Only the **Owner** role can view billing, change plans, open the billing portal, and manage payment methods. **Admin**, **Author**, and **Viewer** roles do not have billing access.

    If you upgraded to a paid plan but cannot see billing options, check your role on **Settings → Team**. An Owner can promote you to Owner if you should manage subscriptions. See [Roles and permissions](/platform/account/roles-and-permissions#permissions-matrix).
  </Accordion>
  ...
  <Accordion title="What is a workflow run?">
    A workflow run is one execution of a workflow for one subscriber. Triggering a workflow to a single subscriber counts as 1 run. Triggering to a topic counts as one run per subscriber whose subscription passes Context and condition filters during fan-out. See [What is a workflow run?](#what-is-a-workflow-run) above, [Billing and workflow runs](/platform/subscription/manage-topic-subscriptions#billing-and-workflow-runs), and [Monitor and debug workflow](/platform/workflow/monitor-and-debug-workflow) for how to inspect runs in the dashboard.
  </Accordion>
