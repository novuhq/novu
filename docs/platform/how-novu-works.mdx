...
```mermaid
flowchart TB
    org[Organization] --> devEnv[Development Environment]
    org --> prodEnv[Production Environment]

    devEnv --> devWorkflows[Workflows]
    devEnv --> devSubscribers[Subscribers]
    devEnv --> devIntegrations[Integrations]
    devWorkflows --> devTrigger[API Trigger]
    devTrigger --> devSteps[Workflow Steps]
    devSteps --> devChannels[Email / SMS / Push / In-App / Chat]
    devIntegrations -.-> devChannels

    prodEnv --> prodWorkflows[Workflows]
    prodEnv --> prodSubscribers[Subscribers]
    prodEnv --> prodIntegrations[Integrations]
    prodWorkflows --> prodTrigger[API Trigger]
    prodTrigger --> prodSteps[Workflow Steps]
    prodSteps --> prodChannels[Email / SMS / Push / In-App / Chat]
    prodIntegrations -.-> prodChannels
```
