...
flowchart TB
    ...
    devWorkflows --> trigger[API Trigger]
    prodWorkflows --> trigger
    trigger --> steps[Workflow Steps]
    steps --> email[Email]
    steps --> sms[SMS]
    steps --> push[Push]
    steps --> inApp[In-App / Inbox]
    steps --> chat[Chat]
    devIntegrations -.-> email
    devIntegrations -.-> sms
    prodIntegrations -.-> push
    prodIntegrations -.-> chat
```
