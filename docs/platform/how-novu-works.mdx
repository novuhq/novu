...
Novu <GlossaryTerm term="trigger">triggers</GlossaryTerm> <GlossaryTerm term="workflow">workflows</GlossaryTerm> that deliver notifications to <GlossaryTerm term="subscriber">subscribers</GlossaryTerm> across in-app, email, SMS, push, and chat <GlossaryTerm term="channel">channels</GlossaryTerm>. Your application sends an <GlossaryTerm term="event">event</GlossaryTerm> to the Novu API, Novu resolves the subscriber and executes each workflow step, then delivers messages through configured <GlossaryTerm term="integration">integrations</GlossaryTerm> or the [`<Inbox />`](/platform/inbox) component.
...
Unlike external channels that rely on third-party <GlossaryTerm term="provider">providers</GlossaryTerm>, the <GlossaryTerm term="inbox">Inbox</GlossaryTerm> is internal to your app. It uses the environment’s <GlossaryTerm term="application-identifier">application identifier</GlossaryTerm> to establish a secure connection, scoped to your subscribers.
...
Everything starts with your <GlossaryTerm term="organization">organization</GlossaryTerm>, the top-level entity in Novu. Each organization can have multiple <GlossaryTerm term="environment">environments</GlossaryTerm>, such as:
...
A workflow defines the <GlossaryTerm term="step">steps</GlossaryTerm> Novu takes to deliver a notification. Steps are either channel steps (email, in-app, push, chat, and SMS) or action steps (Delay and Digest).

Workflows are environment-specific and versioned. You trigger them using the REST API by sending an event along with the relevant secret key from the current environment, `subscriberId`, `workflowId`, and <GlossaryTerm term="payload">payload</GlossaryTerm>.
...
A <GlossaryTerm term="subscriber">subscriber</GlossaryTerm> represents a user in a given environment. Each subscriber has:
...
To send notifications to multiple users, you can group subscribers into a <GlossaryTerm term="topic">topic</GlossaryTerm>. A workflow can be triggered for an individual subscriber or for all subscribers within a topic.
