...
This approach means you can add, remove, or switch providers at any time without having to change your application's code.

<Note>
  Novu sends through one integration per channel step and does not automatically fall back to another provider if that send fails. See the [delivery retry policy](/platform/developer/delivery-retries) for what happens when a provider rejects a message.
</Note>
...
<Card title="Integration conditions" href="/platform/integrations/integration-conditions">
  Route a send to a matching provider integration from subscriber or context data.
</Card>
