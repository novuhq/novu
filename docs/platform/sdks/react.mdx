---
title: "@novu/react"
description: "API reference for the @novu/react SDK components, including Inbox, Bell, Notifications, Subscription, and hooks for notifications and Web Chat."
---
...
| `children` | `ReactNode` | Your own arrangement of the parts. Without it, the built-in arrangement is rendered |
| `className` | `string` | Extra classes for the item root |
| `onNotificationClick` | `NotificationClickHandler` | Falls back to the handler given to `Inbox`, `Notifications` or `InboxContent` |
| `onPrimaryActionClick` | `NotificationActionClickHandler` | Falls back to the handler given to the parent component |
| `onSecondaryActionClick` | `NotificationActionClickHandler` | Falls back to the handler given to the parent component |
| `renderAvatar` | `AvatarRenderer` | Replaces the avatar part |
| `renderSubject` | `SubjectRenderer` | Replaces the subject part |
| `renderBody` | `BodyRenderer` | Replaces the body part |
| `renderDefaultActions` | `DefaultActionsRenderer` | Replaces the read, archive and snooze controls |
| `renderCustomActions` | `CustomActionsRenderer` | Replaces the primary and secondary action buttons |
