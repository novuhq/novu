...
| `conversation` | `AgentConversationRuntime` | Share an existing runtime across hook instances. Do not pass `agentId`, `conversationId`, or `agentHash` with this prop. Get the runtime from `loadAgentChat(novu)` then `novu.agentChat.conversation()`. |
...
| `isLoading` | `boolean` | True while the Agent Chat module loads. For an existing conversation, it stays true until the first history fetch completes. |
...
The same runtime lives on `novu.agentChat.conversation()` in [`@novu/js`](/platform/sdks/javascript#agent-chat). Call `loadAgentChat(novu)` first in JavaScript. The React hook loads Agent Chat for you. Do not call `novu.agentChat.subscribe()` from React UI code.
