...
Use it inside [`NovuProvider`](/platform/sdks/react/hooks/novu-provider). See [Web Chat](/agents/channels/web-chat), [Messages](/agents/channels/web-chat/messages), and [State](/agents/channels/web-chat/messages#state).
...
| `conversationId` | `string` | Resume this conversation and load history on mount. Omit this prop to start a new chat. The first `sendMessage` creates the conversation. See [Start or resume](/agents/channels/web-chat/conversations#start-or-resume). |
...
| `conversationStatus` | `'active' \| 'resolved'` | Conversation lifecycle. The agent sets `resolved` with `ctx.resolve()`. Not a loading flag. See [State](/agents/channels/web-chat/messages#state). |
...
| `retryMessage` | `(messageId: string) => Promise<{ data?: SendMessageResult; error?: NovuError \| WebChatPlanLimitError }>` | Resend a message whose `status` is `failed`. Reuses the original idempotency key. Does not create a second message. See [Retry](/agents/channels/web-chat/conversations#retry). |
...
| `sendAction` | `(args: { actionId: string; sourceMessageId: string; value?: string }) => Promise<{ data?: SendActionResult; error?: NovuError \| WebChatPlanLimitError }>` | Click a Card button. Pass `id` / `value` from the button and `message.id` as `sourceMessageId`. Do not use this for tool approval. See [Cards](/agents/channels/web-chat/generative-ui#cards). |
| `startNewConversation` | `() => void` | Clear the current conversation and start a fresh one without remounting the component. The next `sendMessage` creates the new server conversation. No-op when you pass `conversationId` or a shared `conversation` runtime. See [Start or resume](/agents/channels/web-chat/conversations#start-or-resume). |
...
See [Older messages](/agents/channels/web-chat/conversations#older-messages).
...
See [Reconnect](/agents/channels/web-chat/conversations#reconnect).
...
| `tool` | Tool call and result. See [Tool calls](/agents/channels/web-chat/tools#tool-calls). |
| `approval` | Gated tool waiting on the subscriber. Use `pendingActions` + `respondToAction`. See [Tool approval](/agents/channels/web-chat/tools#tool-approval). |
| `mcp-connection` | MCP server connect card. Open `authorizeUrl` in the browser. See [MCP connect](/agents/channels/web-chat/tools#mcp-connect). |
| `card` | Structured Card tree (`card` object). Render it in your UI. Button clicks call `sendAction`. Agents send Cards with [Interactive cards](/agents/custom-code-agent/building-blocks/reply#interactive-cards). See [Cards](/agents/channels/web-chat/generative-ui#cards). |
| `data` | Custom payload (`name` + `data`). The UI decides how to render it. See [Custom data](/agents/channels/web-chat/generative-ui#custom-data). |
