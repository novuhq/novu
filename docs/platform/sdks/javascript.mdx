...
Two-way in-app chat with an agent. Call `loadAgentChat(novu)` before `novu.agentChat.conversation()`. React apps use [`useAgentChat`](/platform/sdks/react/hooks/use-agent-chat), which loads Agent Chat automatically.
...
```typescript
import { Novu, loadAgentChat } from '@novu/js';
