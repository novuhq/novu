---
title: '@novu/js'
description: "Complete API reference for the @novu/js browser SDK, covering the Novu client, notification streams, preferences, counts, Web Chat, and session management."
---
...
<Tabs>
  ...
  <Note> Learn how Context is used to filter Inbox notifications in the [Inbox with Context](/platform/inbox/configuration/inbox-with-context) guide. </Note>
  ```tsx
  import { Novu } from '@novu/js';
...
<Note>The `seen()` method is only available on individual notification instances, not on the `novu.notifications` object. Use `novu.notifications.seenAll()` for bulk operations.</Note>
...
The client also emits socket lifecycle events so you can surface real-time connection state (for example, a "reconnecting" banner). Each payload carries the `socketUrl` in its `args`.

- `socket.connect.pending`: Triggered when the real-time socket starts connecting.
- `socket.connect.resolved`: Triggered when a connection attempt finishes, whether it succeeds or fails. On failure, the payload includes an `error`.
- `socket.disconnect.resolved`: Triggered when the real-time socket drops or `disconnect()` is called. Use it to show a connection-lost state.
...
novu.on('socket.connect.resolved', ({ args, error }) => {
  if (error) {
    console.error('Real-time connection failed:', args.socketUrl, error);
    return;
  }
  console.log('Real-time connection is live:', args.socketUrl);
});

novu.on('socket.disconnect.resolved', ({ args }) => {
  console.log('Real-time connection dropped:', args.socketUrl);
});
...
It supports creating conditional subscriptions where users only receive notifications when the trigger payload matches a JSON Logic rule (for example, `payload.tier` is `premium`). Subscriber profile, Context data, and step results are not available in subscription conditions. Use [step conditions](/platform/workflow/add-and-configure-steps/step-conditions) for those cases.
...
const { data: subscription } = await novu.subscriptions.create({
  ...
  preferences: [
    ...
    {
      // Conditional filter: JSON Logic evaluated against trigger payload
...
Two-way in-app chat with an agent. Call `loadWebChat(novu)` before `novu.webChat.conversation()`. React apps use [`useWebChat`](/platform/sdks/react/hooks/use-web-chat), which loads Web Chat automatically.

See [Web Chat](/agents/channels/web-chat).
...
Do not call `novu.webChat.subscribe()` from UI code. The runtime owns the socket.
...
| `agentHash` | `string` | HMAC of `agentId`. Required when Web Chat HMAC is on. |
...
See [Messages](/agents/channels/web-chat/messages).
