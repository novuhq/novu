...
description: "Configure the Mailtrap email provider in Novu: generate an API key, verify your sending domain, and send transactional emails through workflows."
---
...
<Note>
  Novu's Mailtrap provider targets Mailtrap's **Email Sending** (production transactional) API — not the **Email Testing** sandbox (`sandbox.api.mailtrap.io`). The integration form accepts an API token and a verified `From` address, and does not expose a sandbox host or inbox ID. To preview outgoing messages without delivering to real recipients, use Novu's [Development environment](/platform/developer/environments) or capture mail with a provider whose sandbox exposes standard SMTP (for example, the [Custom SMTP](/platform/integrations/email/custom-smtp) provider pointed at your test SMTP host).
</Note>
