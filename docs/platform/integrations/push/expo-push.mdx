...
description: "Connect Expo Push to Novu to send push notifications to Expo-managed iOS and Android apps through workflows using Expo push tokens and access credentials."
---
...
Save an Expo override on the Push step when you need Expo Push fields beyond the step subject and body — for example `data`, `sound`, `priority`, or `ttl`. The override is stored on the step, so it is versioned and promoted with the workflow, and every trigger sends it without changing your trigger call.
...
Expo overrides are validated against the [Expo Push message](https://docs.expo.dev/push-notifications/sending-notifications) request format. The editor autocompletes supported field names, validates the JSON, and flags unsupported fields.

- **`to` is managed by Novu.** Novu fills device tokens from the subscriber's credentials — do not set `to` in the override.
- **`body` falls back from the step.** If your override omits `body`, Novu uses the rendered Push step body.
- **`title` is not seeded.** Set `title` in the override when you want one; Novu does not copy the step subject automatically.

See the [Expo Push message format](https://docs.expo.dev/push-notifications/sending-notifications) for the full field list.
