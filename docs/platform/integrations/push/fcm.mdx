...
FCM overrides are validated against the FCM HTTP v1 `Message` shape, plus Novu's multicast `tokens` key. The editor autocompletes supported field names, validates the JSON, and flags unsupported fields.

Supported top-level keys: `notification`, `data`, `android`, `apns`, `webpush`, `fcmOptions`, `token`, `tokens`, `topic`, and `condition`. `token`, `topic`, and `condition` match the HTTP v1 target fields; `tokens` is Novu's multicast alias (Firebase Admin `MulticastMessage`). Step content overrides use the HTTP v1 field names (including `webpush`); trigger examples elsewhere on this page may still show Novu's `webPush` alias.
...
### Routing with `_passthrough`

Routing keys also take effect inside `_passthrough.body`, which is merged last and therefore wins over the same field set on the step or at trigger time. You own the destination when you set these fields.

Because FCM addresses one destination per message, a routing key in `_passthrough.body` replaces the whole set rather than adding to it. In the example below the message goes to `news_updates` only — the `tokens` array is dropped, not sent alongside:
...
Novu ignores a passthrough routing value it cannot address — anything that is not a non-empty string, or for `tokens` an array of non-empty strings — and falls back to the routing set below it. Unlike the schematized keys, `_passthrough` is not validated, so a typo or a wrong type is silently ignored instead of flagged.

The per-subscriber fan-out warning above applies to a `topic` or `condition` set in `_passthrough` too.
