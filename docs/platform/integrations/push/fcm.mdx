...
| `notification` | `object` | Extra keys merged into the notification (title and body are set from the step subject and body). |
| `data` | `object` of `string` values | Custom data payload delivered to the app. FCM requires all values to be strings. |
...
| `type` | `"data"` | Send a data-only message. When set to `"data"`, Novu delivers `title`, `body`, and the trigger `payload` inside `data` instead of `notification`. |
...
To send a silent/data-only message that the OS delivers to your app without showing a system notification, set `type` to `"data"`. Novu moves `title`, `body`, and the trigger `payload` into the message's `data` object:
...
<Note>
  FCM requires every value in `data` to be a string. Novu automatically stringifies non-string payload values before sending.
</Note>
