...
Topic and token routing belong on the trigger. Do not set `topic`, `token`, or `condition` in a step content override — pass those under `overrides.providers.fcm` at [trigger time](/platform/integrations/trigger-overrides) instead. Novu fills device tokens from the subscriber's credentials for normal deliveries.
