...
Register each subscriber's API key as a **channel endpoint** on the Opsgenie integration. Novu encrypts the API key at rest on `ChannelEndpoint.endpoint` and returns the wire shape `{ apiKey, region }` on reads (`region` stays plaintext).
...
sequenceDiagram
  ...
  Note over NovuAPI: API key stored encrypted on endpoint
...
Delete the endpoint to disconnect a subscriber. This removes the encrypted API key stored on the endpoint:
