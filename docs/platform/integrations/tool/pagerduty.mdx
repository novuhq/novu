...
Register each subscriber's routing key as a **channel endpoint** on the PagerDuty integration. Novu encrypts the routing key at rest on the linked channel connection and hydrates the wire shape `{ routingKey, region }` back on reads.
...
sequenceDiagram
  ...
  Note over NovuAPI: routing key stored encrypted
...
Delete the endpoint to disconnect a subscriber. Novu cascades the delete to the underlying channel connection, dropping the encrypted routing key:
