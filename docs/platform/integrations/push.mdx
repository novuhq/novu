...
If your override omits `body`, Novu fills it from the Push step body. `title` is not seeded automatically — set it in the override when you want one.

<Card title="Configure Expo overrides" icon="smartphone" href="/platform/integrations/push/expo-push#provider-content-overrides">
  Schema-backed Expo override fields, body fallback, and title behavior.
</Card>

### Every other push provider is a raw passthrough

For FCM, APNS, OneSignal, Pushpad, Push Webhook, Pusher Beams, and AppIO, the override is an unvalidated escape hatch. Whatever JSON object you save is merged into that provider's outbound payload as-is.
...
- **No key blocklist.** Routing keys such as FCM `topic` are allowed and take effect.
...
Override values may contain [Liquid](/platform/workflow/add-notification-content/personalize-content#apply-logic-with-liquidjs) templates, which Novu compiles at send time. For Expo, if your override omits `body`, Novu fills it with the rendered Push step body. `title` must be set explicitly in the override if you want one.
