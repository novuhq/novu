...
FCM overrides are validated against the FCM HTTP v1 [`Message`](https://firebase.google.com/docs/reference/fcm/rest/v1/projects.messages#Message) shape (`notification`, `data`, `android`, `apns`, `webpush`, `fcmOptions`, `token`, `tokens`, `topic`, `condition`). The editor autocompletes supported field names, validates the JSON, and flags unsupported fields. You may set exactly one of `token`, `tokens`, `topic`, or `condition` for routing; [trigger overrides](/platform/integrations/trigger-overrides) for those keys still take precedence. See [Configure FCM overrides](/platform/integrations/push/fcm#configure-fcm-overrides) for Liquid support and fan-out guidance.
