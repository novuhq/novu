...
FCM overrides are validated against the Firebase Admin SDK [`BaseMessage`](https://firebase.google.com/docs/reference/admin/node/firebase-admin.messaging.basemessage) shape (`notification`, `data`, `android`, `apns`, `webpush`, `fcmOptions`). The editor autocompletes supported field names, validates the JSON, and flags unsupported fields. Routing keys such as `topic` and `token` are not part of the step override schema — use [trigger overrides](/platform/integrations/trigger-overrides) for those.

<Card title="Configure FCM overrides" icon="smartphone" href="/platform/integrations/push/fcm#configure-fcm-overrides">
  Schema-backed FCM override fields, `notification.body` fallback, and routing via trigger overrides.
</Card>
