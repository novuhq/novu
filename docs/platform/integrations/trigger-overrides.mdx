...
<Note>
  Trigger overrides are no longer the only way to reach Block Kit from a dashboard workflow. You can also save Slack overrides directly on the Chat step in the workflow editor, which keeps the layout versioned with the workflow instead of duplicated across every trigger call. Use trigger overrides when the message depends on data your application has at send time.
</Note>
