...
description: "Customize workflow behavior at trigger time in Novu using overrides. Override provider settings, headers, and payloads per event without editing your workflow."
---
...
Some channels also let you save provider overrides on the step itself in the workflow editor, rather than sending them with every trigger. Those configured overrides sit below both scopes above: anything you send at trigger time replaces the configured value for the same field, and fields you do not send keep their configured values. See [chat provider content overrides](/platform/integrations/chat#provider-content-overrides) and [push provider content overrides](/platform/integrations/push#provider-content-overrides).

<Warning>
  **Arrays replace, they do not merge.** When an array key is set at more than one level, the higher-priority array is used whole rather than merged element by element. Previously arrays were merged by index, which could leave a partial array patched over a longer one. Send the complete array at the level you want to win.
</Warning>
...
Use trigger overrides to send [Slack Block Kit](https://api.slack.com/block-kit) messages that only your calling code can build. Pass a `blocks` array under `overrides.steps.<step-id>.providers.slack`, or use `_passthrough.body` for raw Slack API fields.

<Note>
  You can also save Slack overrides directly on the Chat step in the workflow editor, which keeps the layout versioned with the workflow instead of duplicated across every trigger call. That editor is [rolling out gradually](/platform/integrations/chat#provider-content-overrides). Use trigger overrides when the message depends on data your application only has at send time.
</Note>
...
<Card title="Slack Block Kit guide" icon="messages-square" href="/platform/integrations/chat/slack#format-messages-with-block-kit">
  Step-by-step examples for plain text digests, step-configured overrides, direct `blocks` overrides, and `_passthrough` for all supported SDKs.
</Card>
