...
description: 'Route notifications to the right provider integration using dashboard conditions on subscriber, context, and workflow data.'
---
...
Integration conditions can read **subscriber**, **context**, and **workflow** values. Conditions cannot use payload variables or previous step results. Use [step conditions](/platform/workflow/add-and-configure-steps/step-conditions) when conditions depend on workflow-specific payload fields or earlier step output. Existing integrations that already stored `payload.*` rules still evaluate those rules at send time.
...
| `workflow.workflowId` | Route by the workflow trigger identifier |
| `workflow.name` | Route by the workflow display name |
| `workflow.description` | Route by the workflow description |
| `workflow.tags` | Route by workflow tags (array operators such as contains any of) |
| `workflow.severity` | Route by workflow severity |

You can also reference other `context.*` and `subscriber.*` paths, for example `context.tenant.data.plan` or `subscriber.data.region`. Payload paths and custom `workflow.*` paths outside the table are rejected.
...
Create and update accept the same `context.*`, `subscriber.*`, and listed `workflow.*` fields as the dashboard. Payload paths and unknown `workflow.*` paths are rejected on write. Already-saved `payload.*` rules still evaluate at send time.
