...
description: 'Route notifications to the right provider integration using dashboard conditions on subscriber, context, payload, and workflow data.'
---
...
Integration conditions can read **subscriber**, **context**, **payload**, and **workflow** values. They cannot read previous step results. Use [step conditions](/platform/workflow/add-and-configure-steps/step-conditions) to skip a step based on earlier step output.
...
| `payload.*` | Route from trigger payload fields that exist on workflow payload schemas |
...
You can also reference other `context.*`, `subscriber.*`, and `payload.*` paths, for example `context.tenant.data.plan`, `subscriber.data.region`, or `payload.region`. Custom `workflow.*` paths outside the table are rejected.
...
Allowed variables are the same `context.*`, `subscriber.*`, `payload.*`, and listed `workflow.*` fields as in the dashboard. Unknown `workflow.*` paths are rejected.
