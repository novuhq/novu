...
Integration conditions can read **subscriber**, **context**, and **workflow** values. They cannot read trigger payload or previous step results. Use [step conditions](/platform/workflow/add-and-configure-steps/step-conditions) when conditions depend on workflow-specific payload fields or earlier step output.
...
Allowed variables are the same `context.*`, `subscriber.*`, and listed `workflow.*` fields as in the dashboard. Payload paths and unknown `workflow.*` paths are rejected.
