...
async function getTelegramConnectUrl(subscriberId: string) {
  const { url, providerMetadata } = await novu.integrations.linkChannelEndpoint({
  ...
  return {
    connectUrl: url,
    botUsername: providerMetadata?.botUsername,
  };
}
```
