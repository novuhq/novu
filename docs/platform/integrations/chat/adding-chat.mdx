---
title: "Adding Chat Channel Chat Integration with Novu"
sidebarTitle: "Adding Chat Channel"
description: "Add a chat channel provider in Novu to send chat notifications through workflows. Walks through selecting a provider, storing credentials, and testing delivery."
---
...
- (Optional) Add [integration conditions](/platform/integrations/integration-conditions) if this provider should only send when those rules match.
- Click **"Create"**
...
- Configure the Chat content:
  - Use the **block editor** for structured messages (text, images, link buttons), or the **text editor** for a plain-text / Liquid body (for example `{{subscriber.firstName}}, your order {{orderId}} has shipped.`)
  - Dynamic placeholders for personalized content
  - See [Writing chat templates](/platform/integrations/chat/writing-chat-template) for editor details and backward compatibility
