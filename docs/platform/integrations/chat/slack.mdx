...
<Info>
  **Token rotation is supported.** Slack apps with **token rotation** enabled work with Novu: the refresh token and expiry returned during OAuth are stored, and Novu automatically refreshes the bot token before it expires. Apps without token rotation continue to work with their long-lived bot tokens — no changes needed.
  ...
  If you enable token rotation on an app whose workspace was connected **before** rotation was turned on (or before Novu supported it), reconnect the workspace once via OAuth so Novu can capture the refresh token. Until then, deliveries may fail with `token_revoked` or `invalid_auth`.
</Info>
