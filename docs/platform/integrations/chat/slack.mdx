...
await novu.channelConnections.create({
  integrationIdentifier: 'slack',
  subscriberId: 'user-123',        // optional — scope to a subscriber
  context: { tenant: 'acme' },     // optional — scope to a tenant
  workspace: {
...
Provide `subscriberId` or `context` the same way as the Novu-managed OAuth flow.
</Note>
