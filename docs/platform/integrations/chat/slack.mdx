...
<Note>
  The Chat step body is plain text. To send [Slack Block Kit](https://api.slack.com/block-kit) messages, either save a Slack content override on the Chat step in the workflow editor, or pass `blocks` in [trigger overrides](/platform/integrations/trigger-overrides) when you call the API. See [Format messages with Block Kit](#format-messages-with-block-kit).
</Note>
...
Overrides from these sources are combined rather than chosen between. See [How Slack overrides are combined](#how-slack-overrides-are-combined).
...
<Note>
  Editing provider content overrides in the workflow editor is rolling out gradually. If the **Slack** override editor is not visible on your Chat step, contact support@novu.co to have it enabled for your organization. Overrides passed at trigger time work regardless.
</Note>
...
<Steps>
  ...
  <Step title="Open the Slack overrides editor">
    In the step editor, open the provider overrides section and select **Slack**.
  </Step>

  <Step title="Write the override JSON">
    Enter a JSON object of `chat.postMessage` fields. The editor autocompletes the supported field names, including inside `blocks[].elements[]`, and flags invalid JSON and unsupported fields before you can save.
  </Step>

  <Step title="Save the workflow">
    Save the step. The override applies on the next trigger.
  </Step>
</Steps>

Override values may contain [Liquid](/platform/workflow/add-notification-content/personalize-content#apply-logic-with-liquidjs) templates. Novu compiles them at send time against the same variables the step body uses, so `{{payload.*}}`, `{{subscriber.*}}`, and step variables all resolve:
...
Runtime always wins: an override sent at trigger time replaces the value saved on the step for the same field. Fields you do not send at trigger time keep their configured values, so you can save a full Block Kit layout on the step and override only `thread_ts` per trigger.
