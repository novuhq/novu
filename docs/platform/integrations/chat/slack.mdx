...
Novu strips these three from every Slack override before sending, whether they were saved on the step or passed at trigger time, so a stray `channel` cannot redirect a subscriber's message. If you genuinely need to set a raw Slack field that Novu owns, use `_passthrough.body` at trigger time — that is the one deliberate, explicit way through.
...
Use `_passthrough` when you need a value to win unconditionally, or to reach a [Slack `chat.postMessage`](https://api.slack.com/methods/chat.postMessage) field Novu has not yet surfaced. Values in `_passthrough.body` are merged last and take priority over other override fields.
