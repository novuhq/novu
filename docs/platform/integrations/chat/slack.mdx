...
<Info>
  **Token rotation is supported.** Novu stores the refresh token and expiry returned during OAuth and automatically refreshes the bot token before it expires. Apps without token rotation continue to work with their long-lived bot tokens.

  Apps created through Novu's **quick setup** (App Configuration Token) or the setup guide's manifest have token rotation enabled by default. Enabling token rotation on a Slack app is irreversible.
</Info>
...
If deliveries fail with `token_revoked` or `invalid_auth`, reconnect the workspace through OAuth. Reconnecting captures a new refresh token and is required when the workspace was connected before rotation was enabled, before Novu supported rotation, or when the refresh token was regenerated in Slack.
...
### Replace an agent connection's refresh token

If reconnecting a workspace is not practical, open the Slack setup drawer for the agent. Under **Token rotation**, paste a new refresh token from Slack and click **Save & verify**. Novu exchanges and validates the token immediately.

Slack refresh tokens are single-use. Only paste a token that has not been exchanged elsewhere.
