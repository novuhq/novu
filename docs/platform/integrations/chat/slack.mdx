...
<Info>
  ...
  **Fallback: paste a refresh token directly.** If reconnecting isn't practical, open the agent Slack setup drawer and paste the refresh token from Slack's app console under **Token rotation**. Novu exchanges it right away and reports success or failure immediately, so a stale or already-used token surfaces an error instead of silently failing on the next send — Slack refresh tokens are single-use, so only paste one that hasn't already been exchanged elsewhere.
</Info>
