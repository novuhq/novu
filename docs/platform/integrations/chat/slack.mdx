...
For an agent connection, you can replace the refresh token without reconnecting the workspace. Open the Slack setup drawer for the agent, paste a new token under **Token rotation**, and click **Save & verify**. Novu exchanges and validates the token immediately.
