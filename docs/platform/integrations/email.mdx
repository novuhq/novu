---
title: 'E-mail'
description: "Configure the Novu email channel by connecting a provider like SendGrid, Postmark, Mailgun, Amazon SES, or Resend to deliver transactional emails."
---
...
- **Activity tracking**: Track delivery status, open rates, and more in the Novu dashboard. See [Activity Tracking](/platform/integrations/email/activity-tracking).

<Note>
  Email Activity Tracking is available on **Novu Cloud** and **Enterprise self-hosted** only. It is not included in Community self-hosted edition. See [Activity Tracking](/platform/integrations/email/activity-tracking) for more information.
</Note>
...
2. Selects the correct provider. See [Integration conditions](/platform/integrations/integration-conditions).
...
| ------ | ------------------- | --------------------------------------------------------------------------------------------- |
| `file` | `Buffer` \| `string`| The file contents. Provide a `Buffer` or a **pure base64 string** — do not include a `data:<mime>;base64,` prefix. |
| `name` | `string`            | The file name, including its extension (for example `report.pdf`).                            |
| `mime` | `string`            | The MIME type of the file (for example `application/pdf`).                                    |

There is a total limit of 20MB for all attachments included in an email. Attachments are excluded from the 512KB trigger payload limit — see [Payload Limits](/api-reference/payload-limits).

Common MIME types you may need:
...
<Note>
  Attachments belong in `payload`, not `overrides`. The `overrides.email` object is only for the fields listed in [Sending email overrides](#sending-email-overrides) (`bcc`, `cc`, `from`, `replyTo`, `replaceToRecipient`, `senderName`, `text`, `to`) — passing an `attachments` array there has no effect.
</Note>

<Note>
  If your `file` value is a base64 data URL that starts with `data:...;base64,`, strip that prefix before triggering. Novu expects the raw base64 payload only; passing the complete data URL produces invalid attachment content.
</Note>
...
By default, Novu uses your primary email provider. To route automatically from subscriber or context data, see [Integration conditions](/platform/integrations/integration-conditions). To force a specific, active integration for a trigger, use the `integrationIdentifier`.
...
| Goal | Approach |
| --- | --- |
| Two-way agent conversation on email | Connect [email to an agent](/agents/get-started/agents-and-providers) via ACI |
| Forward inbound mail to your backend | [Inbound Email](/platform/inbound-email/overview) with a **Webhook** route → [`email.received`](/platform/developer/webhooks/event-types#email-events) |
| User replies to a workflow email open an agent thread | [Workflow to conversation](/agents/custom-code-agent/concepts#conversations-and-workflows) handoff |

<Note>
  **SMS replies are not supported** in Novu. The SMS channel is outbound-only.
</Note>

<Columns cols={2}>
  <Card icon="mail" href="/platform/inbound-email/overview" title="Inbound Email">
    Verify a domain, add routes, and receive mail on your addresses.
  </Card>
  <Card icon="bot" href="/agents/get-started/agents-and-providers" title="Agents and providers">
    Connect email to an agent for two-way conversations.
  </Card>
</Columns>
...
<Columns cols={2}>
  ...
  <Card title="Anypost" href="/platform/integrations/email/anypost"> Learn how to use the Anypost provider to send emails using Novu.</Card>
