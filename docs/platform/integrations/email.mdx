...
<Note>
  If your `file` value is a base64 data URL that starts with `data:...;base64,`, strip that prefix before triggering. Novu expects the raw base64 payload only. Passing a data URL fails validation with the message: `Each attachment must include file content as a base64-encoded string or Buffer`.
</Note>
