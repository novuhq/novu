---
title: 'How do workflows work in Novu?'
sidebarTitle: 'Workflows'
description: "Learn what Novu workflows are and how they orchestrate multi-channel notification delivery across email, in-app, SMS, push, and chat with steps and controls."
---
...
Workflow and step objects returned by the API include an `origin` field that identifies where the resource was authored. Novu uses this field to decide which bridge endpoint to call when previewing and executing the workflow.

| **Origin**      | **What it means**                                                                                                    |
|-----------------|----------------------------------------------------------------------------------------------------------------------|
| `novu-cloud`    | Created and managed in the current dashboard workflow editor. Runs on Novu's shared bridge endpoint.                  |
| `novu-cloud-v1` | Created with the legacy v1 dashboard editor or v1 workflows API, before the current workflow builder.                 |
| `external`      | Defined in code with the [Novu Framework](/framework). Runs against your own hosted bridge endpoint.                  |
