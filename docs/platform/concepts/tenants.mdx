...
```
  ...
  <Step title="Personalize content per tenant">
    ...
    You can also branch workflow logic on tenant data in [step conditions](/platform/workflow/add-and-configure-steps/step-conditions). To route a tenant to a different provider, see [Integration conditions](/platform/integrations/integration-conditions).
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Can I use a different delivery provider for each tenant?">
    Yes. Add [integration conditions](/platform/integrations/integration-conditions) on `context.tenant.id`, and pass the tenant on the trigger as `context: { tenant: "acme-corp" }`.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Integration conditions" icon="git-branch" href="/platform/integrations/integration-conditions">
    Route each tenant to a different email, SMS, chat, or push integration.
  </Card>
</CardGroup>
