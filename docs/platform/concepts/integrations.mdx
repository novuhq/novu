...
In Novu, <GlossaryTerm term="integration">integrations</GlossaryTerm> are configured connections to third-party services that deliver notifications across supported channels, which are email, SMS, push, and chat.
...
- A <GlossaryTerm term="provider">provider</GlossaryTerm> is the third-party service responsible for sending messages (for example, Twilio, SendGrid, Slack).
- An integration is your instance of that provider, configured with the necessary <GlossaryTerm term="credential">credentials</GlossaryTerm> and settings.
