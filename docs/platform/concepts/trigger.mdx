...
- **Context Upsert:** When the trigger includes context entries with inline `data` objects, the worker creates missing contexts or replaces existing context `data`. String-only context references do not modify stored data. Inbox and other subscriber-facing APIs do not perform context data updates.
