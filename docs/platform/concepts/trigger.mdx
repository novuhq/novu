...
- **Notification Entity Creation:** For each subscriber listed in the trigger event, a corresponding <GlossaryTerm term="notification">notification</GlossaryTerm> entity is created. This entity contains essential data related to the organization, template, subscriber, and event payload.
- **<GlossaryTerm term="job">Job</GlossaryTerm> Creation:** Based on the notification template's defined steps, jobs are generated. These jobs are responsible for carrying out specific tasks related to the event notification. Additionally, the notification entity is updated with a "channels" field generated from these steps, indicating the communication channels through which notifications will be sent.
