...
<AccordionGroup>
  <Accordion title="API Backend">
    ...
    - `REDIS_SKIP_VERSION_CHECK` (default: `false`) Set to `true` when `REDIS_HOST` points at a Redis proxy (for example Predixy in front of Sentinel) that omits `redis_version` in `INFO` responses. Bypasses the BullMQ Redis version check so API and worker services can start.
