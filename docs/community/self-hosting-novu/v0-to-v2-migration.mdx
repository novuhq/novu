...
description: "Migrate a self-hosted Novu instance from the v0 to the v2 Dashboard with steps for data migration, feature flag changes, and workflow updates."
---
...
The changes mechanism was removed, in favor of a more simple "Sync" mechanism, that enables you to sync workflow state between environments. Production is view-only for workflows; edit in Development and publish to Production.

Workflow edits auto-save in Development, with no version history or restore. See [Manage workflows](/platform/workflow/create-a-workflow#manage-workflows).
