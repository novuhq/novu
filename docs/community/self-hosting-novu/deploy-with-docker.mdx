...
- `REDIS_SKIP_VERSION_CHECK` (default: `false`): Set to `true` when Redis is behind a proxy that omits `redis_version` in `INFO` responses. This bypasses the BullMQ version check so API and worker services can start.
