...
<Info>
  For a side-by-side breakdown of what is included in the Community self-hosted edition versus Novu Cloud — including Inbox features, email activity tracking, SSO, RBAC, and compliance certifications — see [Self-Hosted and Novu Cloud](/community/self-hosted-and-novu-cloud). The API reference is the same across both editions; self-hosted deployments simply point at your own API hostname instead of `https://api.novu.co`.
</Info>
