...
<Note>
  A bridge endpoint is only required for workflows built with Novu Framework (code-first workflows). Workflows built in the Dashboard run on Novu's shared bridge endpoint, which Novu manages internally. You don't need to expose, configure, or know the URL of the shared endpoint.
</Note>
...
After you [sync](/framework/deployment/cli) a bridge URL to an environment, Novu shows it in the Dashboard header.

1. Open the [Novu Dashboard](https://dashboard.novu.co) and select the environment you synced (Development or Production).
...
<Frame caption="Click the glowing orb in the top-right navigation bar to view or update your Bridge Endpoint URL.">
  ![Bridge Endpoint URL popover in the Dashboard header](/images/framework/bridge-endpoint-url-dashboard.png)
</Frame>
...
<AccordionGroup>
  ...
  <Accordion title="How can I see my deployed bridge URL?">
    When a bridge URL is set for an environment, open the Novu Dashboard and click the glowing status orb in the top-right navigation bar. The popover shows the current **Bridge Endpoint URL**. See [View your bridge URL](/framework/endpoint#view-your-bridge-url).
  </Accordion>
