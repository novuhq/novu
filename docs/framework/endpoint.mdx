...
Novu Framework requires a **single** `HTTP` endpoint (`/api/novu` or similar) to be exposed by your application. This <Tooltip headline="Bridge" tip="An HTTP endpoint in your application that Novu Cloud calls for Framework workflow discovery, preview, and execution." cta="Learn more" href="/framework/endpoint">Bridge Endpoint</Tooltip> is used to receive events from our **Worker Engine**.
