...
<Note>
  The legacy `--studio-port`, `--studio-host`, and `--no-studio` flags are deprecated and ignored — they belonged to the old Local Studio app, which is now the Dashboard's Local environment.
</Note>
