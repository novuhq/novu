...
Return either a plain-text `body`, a rich `card`, or both. Existing workflows that only return `body` keep working without changes.
...
Build cards with the Chat SDK helpers from `@novu/framework`. The same card model is used for agent replies.
...
- **Description**: Plain-text message body. Use this for simple notifications or as a fallback string alongside a card.

### card

- **Type**: `object` (`CardElement`)
- **Required**: No (required if `body` is omitted)
- **Description**: Structured chat card. Novu renders it to each provider's native format when supported (for example Slack Block Kit or Microsoft Teams Adaptive Cards). Other providers receive a markdown or text fallback derived from the card.
...
| `actions` | `Actions` | Wrapper for buttons and selects. |
| `link-button` | (inside `Actions`) | Button that opens a URL. Requires `label` and `url`. Optional `style`: `primary`, `danger`, `default`. |
| `button` | `Button` | Interactive button with `id` and `label`. Used heavily in [agent replies](/agents/custom-code-agent/building-blocks/reply#interactive-cards). Workflow reply routing for these actions is a separate capability. |
| `select` / `radio_select` | `Select`, `SelectOption` | Choice controls. Same note as interactive buttons for workflow routing. |
| `fields` | | Label/value pairs (`field` children). |
| `table` | | Table with `headers` and `rows`. |
| `section` | | Nested group of child elements. |

The Dashboard [block editor](/platform/integrations/chat/writing-chat-template) authors a common subset of this model (text, image, divider, link buttons, lists, and related layout blocks). Framework can return the fuller card kit above.
