...
In our app, we’ll create a subscriber in Node.js as we’re writing our backend in Node.js, but we also have backend SDKs (Node.js, PHP, [.NET](https://github.com/novuhq/novu-dotnet), Go, Ruby, [Python](https://github.com/novuhq/novu-python), and Kotlin) to choose from. The recommended way to create a subscriber in NodeJS is as follows:
