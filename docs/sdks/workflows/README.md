...
import {
  ...
  invalidateAllWorkflowsList,
} from "@novu/api/react-query/workflowsList.js";
```
...
import {
  ...
  // Utilities to invalidate the query cache for this query in response to
  // mutations and other user actions.
  invalidateWorkflowsGet,
  invalidateAllWorkflowsGet,
} from "@novu/api/react-query/workflowsGet.js";
```
...
import {
  // Mutation hook for triggering the API call.
  useWorkflowsPatchMutation
} from "@novu/api/react-query/workflowsPatch.js";
```
...
<!-- UsageSnippet language="typescript" operationID="WorkflowController_duplicateWorkflow" method="post" path="/v2/workflows/{workflowId}/duplicate" -->
```typescript
import { Novu } from "@novu/api";
...
async function run() {
  const result = await novu.workflows.duplicate({}, "<id>");
...
```typescript
import { NovuCore } from "@novu/api/core.js";
import { workflowsDuplicate } from "@novu/api/funcs/workflowsDuplicate.js";
...
async function run() {
  const res = await workflowsDuplicate(novu, {}, "<id>");
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("workflowsDuplicate failed:", res.error);
  }
}
...
import {
  // Mutation hook for triggering the API call.
  useWorkflowsDuplicateMutation
} from "@novu/api/react-query/workflowsDuplicate.js";
```
...
**Promise\<[operations.WorkflowControllerDuplicateWorkflowResponse](../../models/operations/workflowcontrollerduplicateworkflowresponse.md)\>**
...
import {
  // Mutation hook for triggering the API call.
  useWorkflowsSyncMutation
} from "@novu/api/react-query/workflowsSync.js";
```
