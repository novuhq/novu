...
export function channelConnectionsCreate(
  ...
  >
> {
  return new APIPromise($do(client, createChannelConnectionRequestDto, idempotencyKey, options));
}
...
> {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.ChannelConnectionsControllerCreateChannelConnectionRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.ChannelConnectionsControllerCreateChannelConnectionResponse$inboundSchema, {
      hdrs: true,
      key: 'Result',
    }),
