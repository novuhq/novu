...
export function integrationsGenerateChatOAuthUrl(
  ...
  >
> {
  return new APIPromise($do(client, generateChatOauthUrlRequestDto, idempotencyKey, options));
}
...
> {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.IntegrationsControllerGetChatOAuthUrlRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.IntegrationsControllerGetChatOAuthUrlResponse$inboundSchema, { hdrs: true, key: 'Result' }),
