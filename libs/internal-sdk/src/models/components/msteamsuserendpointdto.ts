/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from 'zod/v3';

export type MsTeamsUserEndpointDto = {
  /**
   * MS Teams user ID
   */
  userId: string;
};

/** @internal */
export type MsTeamsUserEndpointDto$Outbound = {
  userId: string;
};

/** @internal */
export const MsTeamsUserEndpointDto$outboundSchema: z.ZodType<
  MsTeamsUserEndpointDto$Outbound,
  z.ZodTypeDef,
  MsTeamsUserEndpointDto
> = z.object({
  userId: z.string(),
});

export function msTeamsUserEndpointDtoToJSON(msTeamsUserEndpointDto: MsTeamsUserEndpointDto): string {
  return JSON.stringify(MsTeamsUserEndpointDto$outboundSchema.parse(msTeamsUserEndpointDto));
}
