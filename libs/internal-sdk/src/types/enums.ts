...
export function inboundSchema<T extends Record<string, string>>(
  ...
  return z.union([...options.map((x) => z.literal(x)), z.string().transform((x) => unrecognized(x))] as any);
}
...
export function outboundSchema<T extends Record<string, string>>(_: T): z.ZodType<string, z.ZodTypeDef, OpenEnum<T>> {
