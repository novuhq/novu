...
export const scenario: EvalScenario<ConnectFlags> = {
  ...
  tape: connectTape({
    ...
    chunks: [
      ...
      {
        stdout: 'NOVU_CONNECT_AGENT_CHAT_DASHBOARD_URL=https://dashboard.novu.test/env/dev/agents/chat-agent-1/chat',
      },
      {
        stdout: `NOVU_CONNECT_AGENT_CHAT_EMBED_PROMPT_FILE=${path.join(
          projectRoot,
          'novu-connect-agent-chat-embed-prompt.txt'
        )}`,
      },
      {
        stdout: [
