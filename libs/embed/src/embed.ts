...
class Novu {
  ...
  ensureAllowed = () => {
    if (!this.domainAllowed) {
      ...
      const clientIdPlaceholder = clientIdType !== 'string' && clientIdType !== 'number' ? '' : '' + this.clientId;

      throw new DomainVerificationError(`${hostname} is not permitted to use client ID ${clientIdPlaceholder}`);
    }
  };
