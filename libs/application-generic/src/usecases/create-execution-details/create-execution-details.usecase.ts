...
export class CreateExecutionDetails {
  ...
  private async createTraceLogEntry(command: CreateExecutionDetailsCommand, createdAt: string): Promise<void> {
    const traceData = {
      ...
      event_type: this.mapDetailToEventType(command.detail),
  ...
  private mapDetailToEventType(detail: string): EventType {
    const detailToEventTypeMap: Record<string, EventType> = {
      // Step events
    ...
    // Check if detail matches any DetailEnum value
    const eventType = detailToEventTypeMap[detail];
    if (eventType) {
      return eventType;
    }

    // Handle dynamic details like "Message sent: providerId" or "Message created: providerId"
    if (detail.startsWith('Message sent')) {
      return 'message_sent';
    }
    if (detail.startsWith('Message created')) {
