...
import {
  ...
  IPreferenceChannels,
  IPreferenceOverride,
  ISubscriberWorkflowPreferenceResponse,
...
import { GetSubscriberWorkflowPreferenceCommand } from './get-subscriber-workflow-preference.command';
...
@Injectable()
export class GetSubscriberWorkflowPreference {
  constructor(
  ...
  async execute(
    command: GetSubscriberWorkflowPreferenceCommand
  ): Promise<ISubscriberWorkflowPreferenceResponse> {
    const subscriber =
      command.subscriber ??
      ...
      await this.subscriberPreferenceRepository.findOne(
        {
          ...
          _templateId: command.workflow._id,
        },
        'enabled channels',
    ...
    const workflowChannelPreference = command.workflow.preferenceSettings;
    ...
    const { channels, overrides } = overrideWorkflowPreferences(
      {
        template: workflowChannelPreference,
    ...
    return {
      workflow: mapWorkflowConfiguration(command.workflow),
      preferences: {
  ...
  private async getActiveChannels(
    ...
    const initialActiveChannels = filterWorkflowPreference(
      {
        email: true,
        sms: true,
        in_app: true,
  ...
  private async queryActiveChannels(
    ...
    const activeSteps = command.workflow.steps.filter(
...
export function overrideWorkflowPreferences(
...
export const filterWorkflowPreference = (
...
function mapWorkflowConfiguration(
  ...
  return {
    ...
    name: workflow.name,
    tags: workflow?.tags || [],
    critical: workflow.critical != null ? workflow.critical : true,
    triggers: workflow.triggers,
    ...(workflow.data ? { data: workflow.data } : {}),
  };
}
