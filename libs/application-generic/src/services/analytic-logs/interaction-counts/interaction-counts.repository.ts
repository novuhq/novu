...
import {
  INTERACTION_COUNTS_ORDER_BY,
  INTERACTION_COUNTS_TABLE_NAME,
  InteractionCount,
  interactionCountsSchema,
} from './interaction-counts.schema';

@Injectable()
export class InteractionCountsRepository extends LogRepository<
  typeof interactionCountsSchema,
  InteractionCount
> {
  public readonly table = INTERACTION_COUNTS_TABLE_NAME;
  public readonly identifierPrefix = 'ic_';
  ...
  async getTotalInteractionsCount(
    ...
    const currentQuery = `
      ...
      FROM ${INTERACTION_COUNTS_TABLE_NAME}
      WHERE
