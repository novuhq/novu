...
describe("VariableTokenizerService", () => {
	...
	describe("tokenize", () => {
		...
		it("should tokenize variables with helpers", () => {
			...
			expect(result.variableMap.get("[VAR_1]")).toBe(
		...
		it("should handle variables in HTML content", () => {
			...
			expect(result.tokenized).toBe(
				'<div class="greeting">Hello [VAR_1]!</div>',
			);
			expect(result.variableMap.get("[VAR_1]")).toBe("{{name}}");
		});
	...
	describe("detokenize", () => {
		...
		it("should handle round-trip tokenization", () => {
			...
			const restored = service.detokenize(
				tokenized.tokenized,
				tokenized.variableMap,
			);
		...
		it("should handle round-trip with duplicates", () => {
			const original =
				"{{name}} - Welcome {{name}}! Your {{product}} order for {{name}} is ready.";
			const tokenized = service.tokenize(original);
			const restored = service.detokenize(
				tokenized.tokenized,
				tokenized.variableMap,
	...
	describe("validate", () => {
		...
		it("should deduplicate unresolved tokens", () => {
			const result = service.validate(
				"[VAR_1] said hello to [VAR_1] and [VAR_2]",
			);
	...
	describe("edge cases", () => {
		it("should handle special characters in variable names", () => {
			...
			expect(result.variableMap.size).toBe(3);
			expect(service.detokenize(result.tokenized, result.variableMap)).toBe(
				content,
			);
		});
		...
		it("should handle very long content with many variables", () => {
			...
			expect(
				service.detokenize(tokenized.tokenized, tokenized.variableMap),
			).toBe(content);
		});
