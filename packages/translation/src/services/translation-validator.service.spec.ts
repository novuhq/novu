	...
	describe("validate", () => {
		describe("basic validation", () => {
			...
			it("should fail for empty translated content", () => {
				...
				expect(
					result.errors.some(
						(e) => e.type === ValidationErrorType.MISSING_CONTENT,
					),
				).toBe(true);
			});
		...
		describe("token validation", () => {
			it("should fail when unresolved tokens remain", () => {
				...
				expect(
					result.errors.some(
						(e) => e.type === ValidationErrorType.UNRESOLVED_TOKEN,
					),
				).toBe(true);
			});
		...
		describe("HTML tag balance", () => {
			it("should pass for balanced HTML", () => {
				...
				expect(
					result.errors.filter(
						(e) => e.type === ValidationErrorType.HTML_TAG_IMBALANCE,
					).length,
				).toBe(0);
			});
			...
			it("should allow tolerance of 2 for tag imbalance", () => {
				...
				const imbalanceErrors = result.errors.filter(
					(e) =>
						e.type === ValidationErrorType.HTML_TAG_IMBALANCE &&
		...
		describe("broken tag detection", () => {
			...
			it("should pass for valid complex HTML", () => {
				const result = service.validate({
					original:
						'<div class="container" id="main" style="color: red;">Hello</div>',
					translated:
						'<div class="container" id="main" style="color: red;">Hola</div>',
				});
		...
		describe("content length validation", () => {
			it("should warn when translated content is too short", () => {
				const result = service.validate({
					original:
						"This is a long message that should be translated properly.",
					translated: "Hi",
				});
			...
			it("should skip length check for short content", () => {
				const result = service.validate({
					original: "Hi",
					translated:
						"A very long translation that is much longer than the original",
				});
