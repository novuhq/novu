...
export class TranslationValidatorService {
  ...
  private validateTokens(
    ...
    if (variableMap && variableMap.size > 0) {
      ...
      const missingTokens = expectedTokens.filter((token) => !content.includes(token) && !contentTokens.has(token));
  ...
  private analyzeHtmlTags(content: string): {
    ...
    const selfClosingMatches = content.match(HTML_PATTERNS.SELF_CLOSING_TAG) || [];
  ...
  private validateTagBalance(
    original: { openingTags: number; closingTags: number; selfClosingTags: number },
    translated: { openingTags: number; closingTags: number; selfClosingTags: number },
    errors: ValidationError[]
  ): void {
    // Check internal balance of translated content
    const internalImbalance = Math.abs(translated.openingTags - translated.closingTags);
    ...
    if (openingDiff > TAG_BALANCE_TOLERANCE || closingDiff > TAG_BALANCE_TOLERANCE) {
      errors.push({
        type: ValidationErrorType.HTML_TAG_IMBALANCE,
        severity: ValidationSeverity.WARNING,
  ...
  private validateBrokenTags(content: string, errors: ValidationError[]): void {
    ...
    if (reallyBrokenTags.length > 0) {
      errors.push({
        ...
        context: reallyBrokenTags.slice(0, 3).join(', ') + (reallyBrokenTags.length > 3 ? '...' : ''),
      });
    }
   ...
   * @param errors - Array to append errors to
   */
  private checkMalformedPatterns(content: string, errors: ValidationError[]): void {
    ...
    if (malformedAttrTags.length > 0) {
      errors.push({
        ...
        context: malformedAttrTags[0].substring(0, 50) + (malformedAttrTags[0].length > 50 ? '...' : ''),
      });
    }
  }
   ...
   * @param errors - Array to append errors to
   */
  private validateContentLength(original: string, translated: string, errors: ValidationError[]): void {
