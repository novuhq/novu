...
import { DuplicateLocales, DuplicateLocalesResult } from './duplicate-locales.usecase';
...
describe('DuplicateLocales Usecase', () => {
  ...
  describe('resource type handling', () => {
    it('should handle LAYOUT resource type', async () => {
      ...
      localizationGroupRepository.findByResource.mockResolvedValue(layoutSourceGroup);
      localizationGroupRepository.getOrCreateForResource.mockResolvedValue(layoutTargetGroup);
      localizationRepository.find.mockResolvedValue([mockSourceLocalizations[0]]);
      localizationRepository.create.mockResolvedValue({} as any);
