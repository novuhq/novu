...
import { AutoTranslateCommand, LocalizationResourceEnum } from './auto-translate.command';
...
describe('AutoTranslate Usecase', () => {
  ...
  describe('successful auto-translation', () => {
    ...
    it('should skip translation when target locale equals source locale', async () => {
      ...
      settingsRepository.findByOrganization.mockResolvedValue(settingsWithSameLocale);
      localizationGroupRepository.getOrCreateForResource.mockResolvedValue(mockLocalizationGroup);
      localizationRepository.findOne.mockResolvedValue(null);
      localizationRepository.create.mockResolvedValue({} as any);
      ...
      expect(result.results[0].warnings).toContain('Skipped: target locale same as source');
  ...
  describe('locale override', () => {
    it('should use override locales instead of settings', async () => {
      ...
      expect(result.results.map((r) => r.locale)).toEqual(['de_DE', 'it_IT', 'ja_JP']);
    });
