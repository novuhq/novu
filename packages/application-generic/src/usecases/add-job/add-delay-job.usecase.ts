...
export class AddDelayJob {
  ...
  private async calculateDelayAmount(data: JobEntity): Promise<number> {
    ...
    if (data.step.metadata.type === DelayTypeEnum.SCHEDULED) {
      ...
      const noIdenticalDelay = await this.noExistingDelayedJobForDate(
        data,
        delayPath,
        delayDate
      );
    ...
    return AddJob.toMilliseconds(
      data.step.metadata.amount as number,
      data.step.metadata.unit as DigestUnitEnum
    );
  }
