...
describe('MapTriggerRecipientsUseCase', () => {
  ...
  describe('When feature disabled', () => {
    ...
    it('should only process the subscriber id and the subscriber recipients and ignore topics', async () => {
      ...
      await addSubscribersToTopic(
        ...
        firstTopicId,
        firstTopicKey,
        [firstSubscriber, secondSubscriber]
      );
      ...
      const subscribersDefineSubscriberId =
        SubscriberRepository.createObjectId();
  ...
  describe('When feature enabled', () => {
    ...
    it('should map properly a single topic', async () => {
      ...
      const topic = await createTopicEntity(
        session,
        topicRepository,
        topicSubscribersRepository,
        topicKey,
    ...
    it('should map properly multiple topics and deduplicate them', async () => {
      ...
      await addSubscribersToTopic(
        ...
        thirdTopicId,
        thirdTopicKey,
        [firstSubscriber, fourthSubscriber]
      );
...
const addSubscribersToTopic = async (
  ...
  const entities: CreateTopicSubscribersEntity[] = subscribers.map(
    (subscriber) => ({
      _environmentId,
      _organizationId,
  ...
  expect(result?.subscribers).to.have.members(
