...
export function agentMessageToThreadMessage(message: AgentMessage): ThreadMessageLike {
    ...
    switch (part.type) {
      ...
      case 'card': {
        ...
        content.push({
          ...
          data: {
            ...part,
            sourceMessageId: part.sourceMessageId || message.id,
          },
        });
        break;
      }
      ...
      case 'source':
        // Protocol/read-path only — no public agent producer yet; omit from chat UI.
        break;
      case 'file': {
  ...
  let status: ThreadMessageLike['status'] | undefined;
