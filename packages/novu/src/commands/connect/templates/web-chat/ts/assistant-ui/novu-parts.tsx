...
import { field, pressable } from './elements/surfaces';
...
function NovuCard({ data }: { data: AgentCardPart }) {
  ...
  async function clickButton(actionId: string, value?: string) {
    const sourceMessageId = data.sourceMessageId?.trim();
    if (busyId || !sourceMessageId) return;
    setBusyId(actionId);
    try {
      await sendAction({ actionId, sourceMessageId, value });
    } finally {
      setBusyId(undefined);
    }
  }
