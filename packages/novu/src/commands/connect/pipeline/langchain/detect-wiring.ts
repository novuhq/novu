...
const LANGCHAIN_IMPORT = /@novu\/framework\/langchain/;
const BRIDGE_ROUTE_PATTERNS = [/api[/\\]novu[/\\]route\.(ts|tsx|js|jsx|mjs)$/];

export type LangChainWiringDetection = {
  hasLangChainImport: boolean;
...
export function detectLangChainWiring(projectDir: string): LangChainWiringDetection {
  const resolvedDir = path.resolve(projectDir);
  const files = listSourceFiles(resolvedDir);

  let hasLangChainImport = false;
  ...
  for (const filePath of files) {
    ...
    if (LANGCHAIN_IMPORT.test(contents)) {
  ...
  return {
    hasLangChainImport,
    hasBridgeRoute,
    isWired: hasLangChainImport && hasBridgeRoute,
  };
}
