...
export async function offerPostConnectBridgeTunnel(input: {
  ...
  agentChatHandoff?: ConnectAgentChatHandoff;
  agentChatProjectDir?: string;
  ci?: boolean;
}): Promise<boolean> {
  if (input.ci || !input.agentChatHandoff || !isBridgeConnectMode(input.connectMode)) {
    return false;
  }
  ...
  const projectDir = input.agentChatProjectDir ?? bridgeOutcome.projectDir;
  const adapter =
