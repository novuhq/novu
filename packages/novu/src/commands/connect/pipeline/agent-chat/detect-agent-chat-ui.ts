...
const USE_AGENT_CHAT = /\buseAgentChat\b/;

export type AgentChatUiWiringDetection = {
  hasNovuReact: boolean;
  hasUseAgentChat: boolean;
  isWired: boolean;
};
...
export function detectAgentChatUiWiring(projectDir: string): AgentChatUiWiringDetection {
  const resolvedDir = path.resolve(projectDir);
  const files = listSourceFiles(resolvedDir);

  let hasNovuReact = false;
  let hasUseAgentChat = false;
  ...
  for (const filePath of files) {
    ...
    if (USE_AGENT_CHAT.test(contents)) {
      hasUseAgentChat = true;
    }
  }
  ...
  return {
    hasNovuReact,
    hasUseAgentChat,
    isWired: hasNovuReact && hasUseAgentChat,
  };
}
