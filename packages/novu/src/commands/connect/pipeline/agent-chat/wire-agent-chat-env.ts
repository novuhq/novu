...
export type AgentChatEnvMergeResult = {
  envPaths: string[];
  updatedKeys: string[];
};
...
export function buildAgentChatEnvEntries(input: AgentChatEnvInput): Record<string, string> {
...
function serializeEnvWithComment(existingContents: string, entries: Map<string, string>, created: boolean): string {
  if (created || !existingContents.trim()) {
    const lines = ['# Agent Chat (added by npx novu connect)'];
  ...
  const preserved = lines.filter((line) => {
    ...
    return !(AGENT_CHAT_ENV_KEYS as readonly string[]).includes(key);
  });

  preserved.push('', '# Agent Chat (added by npx novu connect)');
  for (const key of AGENT_CHAT_ENV_KEYS) {
    const value = entries.get(key);
    if (value) {
...
export function mergeAgentChatEnv(input: AgentChatEnvInput): AgentChatEnvMergeResult {
  const envPaths = resolveProjectEnvPaths(input.projectDir);
  const entriesToSet = buildAgentChatEnvEntries(input);
