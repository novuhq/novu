...
import type { AgentChatSetupMode } from '../../types';
...
export function AgentChatHandoffContent({
  dashboardUrl,
  onContinue,
}: {
  dashboardUrl: string;
...
export function AgentChatSetupPickContent({
  projectKind,
  onResolve,
}: {
...
export function AgentChatInkSuccessContent({
  result,
  embedPrompt,
  embedPromptFile,
  onDismiss,
  ...
  const presentation = resolveAgentChatSuccessPresentation(result);
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        <AgentChatEmbedFinishContent
          ...
          envPaths={result.agentChatOutcome?.envPaths}
  ...
  if (presentation?.kind === 'merged-scaffold') {
    return (
      <Box flexDirection="column" gap={1}>
        <Text color="green">✓ Agent app ready with Agent Chat.</Text>
        <Text>
        ...
        <Text color="cyan">Agent Chat UI {presentation.chatUrl}</Text>
  ...
  if (presentation?.kind === 'standalone-scaffold') {
    return (
      <Box flexDirection="column" gap={1}>
        <Text color="green">✓ Agent Chat app ready.</Text>
        <Text>
  ...
  if (presentation?.kind === 'generic-linked') {
    return (
      <Box flexDirection="column" gap={1}>
        <Text color="green">✓ Agent Chat linked — add it to your app.</Text>
