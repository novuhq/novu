...
program
  ...
  .option(
    '--runtime <runtime>',
    `Agent connect mode (${AGENT_CONNECT_MODES.join(' | ')}). Defaults to demo for managed agents; custom-code reuses a self-hosted agent; chat-sdk provisions a bridge agent`
  )
  .option(
    '--agent-identifier <id>',
