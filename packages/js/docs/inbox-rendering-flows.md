...
7. `NovuUI` now provides `{ novuUI, icons }` and renders the outermost `OutletScope` (`react/components/OutletScope.tsx`) around its children, on the store it created for the icon overrides. A scope renders its children first and an `OutletHost` after them, so any render prop registered during the render is current when the outlets render. `DefaultInbox` uses that outermost scope, since nothing from the host app can wrap it. The components a host can wrap in children mode, `Bell`, `Notifications` and `InboxContent`, each render a nested scope of their own, because React events from portal content propagate along the React tree, not the DOM: with every portal under `NovuUI`, a custom bell rendered inside a host's popover trigger sat outside the trigger's React subtree and its clicks never reached the trigger.
