...
export const Settings = () => {
  ...
  return (
    <div class="nt-p-2 nt-flex nt-flex-col nt-gap-1 nt-bg-background nt-overflow-y-auto">
...
const ChannelsLabel = (props: { channels: ChannelPreference }) => {
  ...
  return <div class="nt-text-sm nt-text-foreground-alpha-600 nt-text-start">{definedKeys()}</div>;
};
...
const SettingsRow = (props: {
  ...
  return (
    <div class="nt-p-4 nt-flex nt-flex-col nt-gap-1 nt-items-start nt-self-stretch">
      <div
        class="nt-flex nt-justify-between nt-flex-nowrap nt-self-stretch nt-cursor-pointer nt-items-center"
        onClick={() => setIsOpen((prev) => !prev)}
      >
        <div>
          <div class="nt-text-base nt-font-semibold nt-text-foreground nt-text-start">{props.label}</div>
      ...
      <Show when={isOpen()}>
        <div class="nt-flex nt-flex-col nt-gap-1 nt-self-stretch">
          <For each={channels()}>
            {(channel) => (
              <ChannelRow
