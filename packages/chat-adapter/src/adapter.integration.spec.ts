...
function bridgeRequest(
  overrides: Partial<AgentBridgeRequest> = {},
): AgentBridgeRequest {
  return {
    version: 1,
...
async function deliver(
  adapter: ReturnType<typeof createNovuAdapter>,
...
describe("Novu adapter end-to-end", () => {
  ...
  beforeEach(() => {
    fetchMock = vi.fn(
      async () =>
        new Response(
          JSON.stringify({ messageId: "m-1", platformThreadId: "t-1" }),
          { status: 200 },
        ),
    );
  });
  ...
  it("normalizes a chat-sdk Card posted by a handler into reply.card", async () => {
    ...
    expect(payload.reply.card.children).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: "text", content: "Hello from a card" }),
      ]),
    );
  });
