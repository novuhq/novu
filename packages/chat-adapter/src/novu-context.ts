...
export function getNovuContext(thread: { id: string; adapter: unknown }): NovuContext {
  ...
  return {
    ...
    getConversation: () => source.getConversation(threadId),
    getHistory: () => source.getHistory(threadId),
    getEmailContext: () => source.getEmailContext(threadId),
    getMetadata: async (key) => {
      const conversation = await source.getConversation(threadId);
