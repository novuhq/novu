 ...
 *   });
 */
export function getNovuContext(thread: {
  id: string;
  adapter: unknown;
}): NovuContext {
  ...
  if (
    ...
    throw new Error(
      "getNovuContext() requires a thread owned by the Novu adapter",
    );
  }
  ...
  return {
    ...
    getEmailContext: async () =>
      (await snapshot())?.platformContext?.email ?? null,
    getMetadata: async (key) =>
      (await snapshot())?.conversation.metadata?.[key],
    ...
    setMetadata: (key, value) =>
      emit({ type: "metadata", action: "set", key, value }),
