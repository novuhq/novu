...
export async function createApp({
  ...
  if (example) {
    ...
    if (repoUrl) {
      ...
      if (!repoInfo) {
        console.error(`Found invalid GitHub URL: ${red(`"${example}"`)}. Please fix the URL and try again.`);
        process.exit(1);
      }
      ...
      if (!found) {
        console.error(
          `Could not locate an example named ${red(`"${example}"`)}. It could be due to the following:\n`,
