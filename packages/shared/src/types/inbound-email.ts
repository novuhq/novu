 ...
 * delivery path; URL-only inbound webhooks use a six-hour expiration. `content` and
 * `contentBytes` are kept for one release cycle so existing consumers do not break; both
 * fields are `@deprecated` and will be removed in the next major version.
 *
