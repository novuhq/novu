...
class NotImplementedError extends Error {
  constructor(method: string, reason: string) {
    super(`${method} is not supported by the agent chat adapter: ${reason}`);
    this.name = 'NotImplementedError';
  }
}

type MessageConstructor = new (data: unknown) => Message<AgentChatRawMessage>;
 ...
 */
export class NovuAgentChatAdapterImpl implements Adapter<AgentChatThreadId, AgentChatRawMessage> {
  ...
  private readonly config: AgentChatAdapterConfig;
  private chat: ChatInstance | null = null;
  private MessageClass: MessageConstructor | null = null;
  private parseMarkdownFn: ((md: string) => Root) | null = null;

  constructor(config: AgentChatAdapterConfig) {
    this.config = config;
    this.userName = config.userName ?? 'agent-chat-agent';
  }
  ...
  encodeThreadId(data: AgentChatThreadId): string {
    return toThreadId(data.conversationId);
  }

  decodeThreadId(threadId: string): AgentChatThreadId {
    const conversationId = conversationIdFromThreadId(threadId);
    if (!isValidConversationId(conversationId)) {
      throw new Error(`Invalid agent chat thread id: ${threadId}`);
    }
  ...
  private async verifySession(request: Request): Promise<AgentChatSession | Response> {
    try {
      ...
      console.error('[chat-adapter-agent-chat] verifySession threw', err);

      return jsonResponse({ message: 'Unauthorized' }, 401);
    }
  }

  private async parseRequestBody(request: Request): Promise<AgentChatRequestBody | Response> {
    try {
      return (await request.json()) as AgentChatRequestBody;
    } catch {
  ...
  private resolveIngressKind(body: AgentChatRequestBody): IngressKind | Response {
  ...
  private resolveResumeConversationId(body: AgentChatRequestBody): string | null | 'invalid' {
    const raw =
  ...
  parseMessage(raw: AgentChatRawMessage): Message<AgentChatRawMessage> {
  ...
  async postMessage(threadId: string, message: AdapterPostableMessage): Promise<RawMessage<AgentChatRawMessage>> {
  ...
  ): Promise<RawMessage<AgentChatRawMessage>> {
  ...
  async fetchThread(threadId: string): Promise<ThreadInfo> {
    return {
      id: threadId,
      channelId: threadId,
      metadata: { title: 'Agent chat' },
    };
  }

  async fetchMessages(_threadId: string): Promise<FetchResult<AgentChatRawMessage>> {
    return { messages: [] };
  }
  ...
  async addReaction(_threadId: string, _messageId: string, _emoji: unknown): Promise<void> {
    throw new NotImplementedError('addReaction', 'agent chat has no platform reaction API');
  }

  async removeReaction(_threadId: string, _messageId: string, _emoji: string): Promise<void> {
    throw new NotImplementedError('removeReaction', 'agent chat has no platform reaction API');
  }
