...
import type { AgentChatAdapterConfig, AgentChatSession } from './types.js';
...
function createConfig(overrides: Partial<AgentChatAdapterConfig> = {}): AgentChatAdapterConfig {
  return {
...
async function createAdapter(config: AgentChatAdapterConfig = createConfig()) {
  const adapter = new NovuAgentChatAdapterImpl(config);
...
function jsonRequest(body: unknown, headers?: Record<string, string>): Request {
  return new Request('https://api.novu.test/v1/agent-chat/conversations', {
    method: 'POST',
...
describe('NovuAgentChatAdapterImpl', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('dispatches processMessage then returns 201', async () => {
    ...
    expect(processMessage.mock.calls[0]?.[1]).toBe(`agent_chat:${body.data.identifier}`);
  ...
  it('resumes with conversationIdentifier when authorizeResume allows', async () => {
    ...
    expect(processMessage.mock.calls[0]?.[1]).toBe('agent_chat:conv_abcdefghijkl');
  });
  ...
  it('dispatches processAction then returns 200 for actionId ingress', async () => {
    ...
    expect(processAction.mock.calls[0]?.[0]).toMatchObject({
      ...
      value: 'Approve once',
      threadId: 'agent_chat:conv_abcdefghijkl',
      user: { userId: 'sub_1' },
    });
  });
  ...
  it('postMessage delegates to deliverMessage without inventing mongo semantics', async () => {
    ...
    const sent = await adapter.postMessage('agent_chat:conv_abcdefghijkl', { markdown: 'Agent hi' });
  ...
  it('postMessage extracts plain text from bare CardElement posts', async () => {
    ...
    await adapter.postMessage('agent_chat:conv_abcdefghijkl', {
      type: 'card',
  ...
  it('editMessage and deleteMessage delegate to injected callbacks', async () => {
    ...
    await adapter.editMessage('agent_chat:conv_abcdefghijkl', 'act_message0001', { markdown: 'edited' });
    await adapter.deleteMessage('agent_chat:conv_abcdefghijkl', 'act_message0001');
  ...
  it('declares history off, thread lock scope, and typing via injected callback', async () => {
    ...
    await adapter.startTyping('agent_chat:conv_abcdefghijkl', 'Thinking...');
    expect(config.startTyping).toHaveBeenCalledWith({
      threadId: 'agent_chat:conv_abcdefghijkl',
      status: 'Thinking...',
    });
    expect(adapter.encodeThreadId({ conversationId: 'conv_abcdefghijkl' })).toBe('agent_chat:conv_abcdefghijkl');
    expect(adapter.decodeThreadId('agent_chat:conv_abcdefghijkl')).toEqual({ conversationId: 'conv_abcdefghijkl' });
  });
  ...
  it('declares supportsClientMessageIds and forwards an embedded messageId to deliverMessage', async () => {
    ...
    const sent = await adapter.postMessage('agent_chat:conv_abcdefghijkl', {
      markdown: 'Agent hi',
  ...
  it('stopTyping delegates to startTyping callback without a status', async () => {
    ...
    await adapter.stopTyping('agent_chat:conv_abcdefghijkl');

    expect(config.startTyping).toHaveBeenCalledWith({ threadId: 'agent_chat:conv_abcdefghijkl' });
  });
});
