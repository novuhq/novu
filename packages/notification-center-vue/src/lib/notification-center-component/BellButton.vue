...
export default {
  data() {
    const color = this.colors.unseenBadgeColor;
    const regularColor = typeof color === 'string';

    return {
      gradientDot: {
        stopColor: regularColor ? (color as string) : (color as ISvgStopColor).stopColor,
        offsetStopColor: regularColor ? (color as string) : (color as ISvgStopColor).stopColorOffset,
      },
      bellButtonClasses: this.getBellButtonClasses(this.bellButtonClass),
      gradientDotClasses: this.getBellButtonDotClasses(this.gradientDotClass),
    };
  },
  watch: {
    bellButtonClass: {
      handler(bellButtonClass) {
        this.bellButtonClasses = this.getBellButtonClasses(bellButtonClass);
      },
    },
    gradientDotClass: {
      handler(gradientDotClass) {
        this.gradientDotClasses = this.getBellButtonDotClasses(gradientDotClass);
      },
    },
  },
  methods: {
    getBellButtonClasses(bellButtonClass: string) {
      return {
        ...(bellButtonClass && { [bellButtonClass]: true }),
        'nc-bell-button': true,
      };
    },
    getBellButtonDotClasses(gradientDotClass: string) {
      return {
        ...(gradientDotClass && { [gradientDotClass]: true }),
        'nc-bell-button-dot': true,
      };
    },
  },
};
...
<template>
  <!-- @ts-ignore -->
  <button :class="bellButtonClasses">
    <svg
      width="30"
      height="30"
      viewBox="0 0 30 30"
