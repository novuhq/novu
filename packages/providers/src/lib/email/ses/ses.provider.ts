...
function buildMessageId(body: Record<string, unknown>) {
  ...
  const sourceArnRegion = (mailData.sourceArn as string).split(':')[3];
  /*
   * sometimes the payload is missing the sourceArn, so we fallback to the TopicArn instead
   * example arn:aws:sns:us-east-1:123456789012:test-topic
   */
  const topicArnRegion = (body.TopicArn as string).split(':')[3];

  const region = sourceArnRegion || topicArnRegion;

  if (mailData.messageId && region) {
