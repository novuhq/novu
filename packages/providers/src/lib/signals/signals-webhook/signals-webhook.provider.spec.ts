...
import { SignalsWebhookProvider } from './signals-webhook.provider';
...
beforeAll(() => {
  ...
  vi.spyOn(dns.promises, 'lookup').mockImplementation(((hostname: string, opts: any): any => {
    if (hostname === 'test-signals-webhook.invalid') {
...
beforeEach(async () => {
  ...
  serverUrl = `http://test-signals-webhook.invalid:${addr.port}/webhook`;
});
