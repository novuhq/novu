...
export function DebugInspector({ events, onClear }: { events: DebugEvent[]; onClear: () => void }) {
  ...
  return (
    <div className="debug-console" data-open={open}>
      <button
        type="button"
      ...
      >
        ...
        <span className="debug-trigger-count">{events.length}</span>
      </button>

      {open ? (
        <aside className="panel debug-panel" id="debug-panel" aria-label="Event inspector">
          ...
          <ol className="debug-list">
            {filtered.length === 0 ? (
              ...
              filtered.map((event) => {
                const expandedEvent = expanded === event.id;
                return (
                  <li key={event.id} className={`debug-item source-${event.source}`}>
                    <button
                      type="button"
                      className="debug-row"
                      onClick={() => setExpanded(expandedEvent ? null : event.id)}
                    ...
                    {expandedEvent && event.payload !== undefined ? (
