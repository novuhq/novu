...
import { runOrigin, useRunActivity } from '../lib/run-activity';
import { getSocketStatus, type SocketStatus, setSocketStatus, subscribeSocketStatus } from '../lib/socket-status';
...
function ChatSurface({
  ...
  const agentChat = useAgentChat({ agentId, conversationId, onActionRequested, onMessage, onEvent });
  const {
    messages,
    pendingActions,
    sendMessage,
  ...
  // Present once packages include NV-8569 (`channel.typing` → `useAgentChat().typing`).
  const typing = 'typing' in agentChat ? (agentChat as { typing?: { status?: string } }).typing : undefined;
  ...
  const origin = runOrigin(isRunning, lastTransition);
    ...
    <>
      ...
      <div className="chat-main">
        <header className="chat-topbar">
          ...
          {activeConversationId ? <code>{activeConversationId}</code> : null}
        </header>
        {error ? (
